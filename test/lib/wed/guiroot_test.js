...
describe("guiroot", function guirootBlock() {
  ...
  describe("GUIRoot", function GUIRoot() {
    ...
    it("fails if the node is already marked", function test() {
      assert.throws(
        function fn() {
          new guiroot.GUIRoot($root[0]); // eslint-disable-line no-new
        },
        window.Error, "node already marked as root");
    });
    ...
    describe("nodeToPath", function nodeToPath() {
      ...
      it("returns a correct path on phantom_wrap nodes", function test() {
      ...
      it("returns a correct path on attribute", function test() {
        ...
        assert.equal(root_obj.nodeToPath(node), "0/1/0/1/1/@type");
      });

      it("returns a correct path on text node in attribute", function test() {
        var node = defined($root.find(".body>.p:last-of-type>.quote")[0]);
        // Decorate it.
        $(node).prepend(
          "<span class=\"_gui _phantom __start_label _quote_label" +
      ...
      it("returns a correct node on attribute path", function test() {
        ...
        $(node).prepend(
          "<span class=\"_gui _phantom __start_label _quote_label" +
            ...
            "</span> >&nbsp;</span></span>");
        node = defined($(node).find("._attribute_value")[0]);
        assert.equal(root_obj.pathToNode("0/1/0/1/1/@type"), node);
      });
