...
<xsl:stylesheet
  ...
  <xsl:template name="handleNamespaceChanges">
    ...
    <!-- What we are doing here is getting the prefixes whose
         definition the current node changes. At root all prefixes are
         new, otherwise we need to check against the parent node. The
         variable prefixes will contain all the namespaces for which
         we have to produce attributes. We exclude the xml prefix
         because it has a value which is determined by the XML
    ...
    <xsl:for-each select="$prefixes">
      ...
      <xsl:attribute name="data-wed-xmlns{if (. = '') then '' else concat('---', replace(., '--(-+)', '---$1'))}"
                     select="namespace-uri-for-prefix(., $current-node)"/>
    </xsl:for-each>
  </xsl:template>
