/**
 * True if the browser is Edge.
 */
export declare const EDGE: boolean;
/**
 * True if the browser is Chrome.
 */
export declare const CHROME: boolean;
/**
 * True if the browser is Internet Explorer up to version 10.
 */
export declare const MSIE_TO_10: boolean;
export declare const MSIE_11_AND_UP: boolean;
/**
 * True if the browser is Internet Explorer, any version.
 */
export declare const MSIE: boolean;
/**
 * True if the browser is Firefox.
 */
export declare const FIREFOX: boolean;
/**
 * True if the browser is Firefox 24. This is an ESR version.
 */
export declare const FIREFOX_24: boolean;
/**
 * True if the browser is Gecko-based.
 */
export declare const GECKO: boolean;
/**
 * True if running on a OS X system.
 */
export declare const OSX: boolean;
/**
 * True if running on Windows.
 */
export declare const WINDOWS: boolean;
