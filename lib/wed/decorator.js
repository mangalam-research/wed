...
Decorator.prototype.elementDecorator = function (
    ...
    this.setReadOnly(el, data_node.wed_possible_due_to_wildcard);
...
Decorator.prototype._contextMenuHandler = function (
    ...
    function processAttributeNameEvent(ev, element) {
        if (ev.params[1].simple()) {
            ...
            for (var name_ix = 0, name; (name = names[name_ix]); ++name_ix) {
    ...
    if (attr_val) {
        ...
        if (!editor.isAttrProtected(data_node)) {
            ...
            for(tr_ix = 0, tr; (tr = trs[tr_ix]) !== undefined; ++tr_ix) {
