...
define(/** @lends module:decorator */ function f(require, exports) {
  ...
  var $ = require("jquery");
  var isText = require("./domtypeguards").isText;
  ...
  /**
   * Generic handler for setting ``contenteditable`` on nodes
   * included into the tree.
   */
  ...
  Decorator.prototype.elementDecorator = function elementDecorator(
    ...
    pre_ph.innerHTML = "&nbsp;<span class='_phantom _element_name'>" +
      ...
      "<span class='_phantom _greater_than'> >&nbsp;</span>";
    ...
    post_ph.innerHTML = "<span class='_phantom _less_than'>&nbsp;&lt; </span>" +
      "<span class='_phantom _element_name'>" +
      orig_name + "</span>&nbsp;";
  ...
  Decorator.prototype._contextMenuHandler = function _contextMenuHandler(
    ...
    if (attr_val) {
      ...
      if (!top_node) {
        trs = editor.getElementTransformationsAt(tree_caret, "insert");
        for (tr_ix = 0; tr_ix < trs.length; ++tr_ix) {
          tr = trs[tr_ix];
        ...
        if (at_start) {
          ...
          // possibilities.
          var caretInside =
              tree_caret.make(tree_caret.node.childNodes[tree_caret.offset], 0);
          trs = editor.getElementTransformationsAt(caretInside, "wrap-content");
          for (tr_ix = 0; tr_ix < trs.length; ++tr_ix) {
            tr = trs[tr_ix];
            if (tr.name !== undefined) {
              // Regular case: we have a real transformation.
              pushItem({ name: tr.name, node: node }, tr.tr, undefined);
            }
            else {
              // It is an action rather than a transformation.
              pushItem(undefined, tr.tr);
