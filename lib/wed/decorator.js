...
(function () {
     ...
     * <code>contentDecoratorInclusionHandler</code>
     */
    this.contentDecoratorChildChangeHandler = function ($root,
                                                        $added,
                                                        $removed,
                                                        $target) {
        if (($target.closest(document.documentElement).length >
            0) && ($added.is("._real, ._phantom_wrap") ||
                  $removed.is("._real, ._phantom_wrap"))) {
            this.contentDecoratorInclusionHandler($root, $target);
        }
    };
    ...
    this.contentDecoratorTextHandler = function ($root,
        ...
        if ($target.closest(document.documentElement).length >
            0)
            this.contentDecoratorInclusionHandler($root,
                                                  $target.parent());
    };
    ...
    this.contentDecoratorInclusionHandler = function ($root,
                                                       $element) {
        this._contentDecorator($root, $target,
                               null, null);
    };
    ...
    this._contentDecorator = function ($root, $target,
        ...
        if (first_index === 0) {
            if ((first_index >= target.childNodes.length) ||
                ((target.childNodes[first_index].nodeType
                  !== Node.TEXT_NODE) &&
                 !$(target.childNodes[first_index]).is("._text")))
                $target.prepend(domutil.makePlaceholder());
        }
        else if (node_before.nodeType !== Node.TEXT_NODE &&
                 (first_index < target.childNodes.length) &&
                 (target.childNodes[first_index].nodeType !==
                  Node.TEXT_NODE) &&
                 !$(target.childNodes[first_index]).is("._text"))
            $node_before.after(domutil.makePlaceholder());

        // Last index could change due to the previous update.
        ...
        // Make the placeholder we've already put invisible
        $target.children('._placeholder').addClass('_invisible');

        if (last_index === target.childNodes.length - 1) {
            if (target.childNodes[last_index].nodeType !==
                 Node.TEXT_NODE &&
               !$(target.childNodes[last_index]).is("._text")) {
                $target.append(domutil.makePlaceholder().addClass('_invisible'));
                last_index++;
            }
        }
        else if ((node_after.nodeType !== Node.TEXT_NODE) &&
                 (last_index > 0) &&
                 (target.childNodes[last_index].nodeType !==
                  Node.TEXT_NODE) &&
                 !$(target.childNodes[last_index]).is("._text")) {
            $node_after.before(domutil.makePlaceholder().addClass('_invisible'));
            last_index++;
        }

        for(var ix = first_index; ix < last_index; ix++) {
            var $cur_child = $(target.childNodes[ix]);
            // Text nodes never get anything before or after
            if ((target.childNodes[ix].nodeType === Node.TEXT_NODE) ||
                $cur_child.is("._text"))
                continue;

            if (($cur_child.is("[contenteditable='true']") &&
                 $(target.childNodes[ix + 1]).is(
                     "[contenteditable='true']")))
                $cur_child.after(domutil.makePlaceholder().addClass('_invisible'));
        }
    };
