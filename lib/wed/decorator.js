...
Decorator.prototype._contextMenuHandler = function (
    at_start, wed_ev, wed_jQthis, ev, jQthis) {
    var node = $(wed_jQthis).parents('._real').first().get(0);
    var menu_items = [];
    ...
    if (node.parentNode !== editor.gui_root) {
        ...
        var handler = function(e) {
            editor.setDataCaret(tree_caret);
        ...
        editor.validator.possibleAt(
            tree_caret[0], tree_caret[1]).forEach(function (ev) {
                ...
                var trs = mode.getContextualActions("insert", unresolved);
                if (trs === undefined)
                    return;
        ...
        var pos = $(editor.menu_layer).offset();
        editor.displayContextMenu(ev.pageX - pos.left,
                                  ev.pageY - pos.top,
                                  menu_items);
        return false;
    }
