...
Decorator.prototype.elementDecorator = function (
    ...
    var gui_caret = this._editor.getGUICaret(true);
    var data_caret;
    if (gui_caret && this._editor.gui_root.contains(gui_caret.node)) {
        data_caret = this._editor.getDataCaret();
        if (data_caret &&
            !(data_caret.node instanceof this._editor.my_window.Attr &&
              data_caret.node.ownerElement === $(el).data("wed_mirror_node")))
    ...
    for(var i = 0, remove; (remove = to_remove[i]); ++i)
    ...
    if (this._editor.attributes === "show" ||
        ...
        Object.keys(attributes).sort().forEach(function (name) {
            var name_el = '<span class="_phantom _attribute_name">' + name +
                '</span>';
            var value_el = '<span class="_phantom _attribute_value">' +
                domutil.textToHTML(attributes[name]) + '</span>';
            attributes_html.push('<span class="_phantom _attribute">' +
                                 name_el + '="' + value_el + '"</span>');
        });
    }
...
Decorator.prototype._contextMenuHandler = function (
    ...
    if (attr_val) {
        ...
        var decoded = util.decodeAttrName(data_node.name);
        if (!editor.isAttrProtected(data_node)) {
            var trs = mode.getContextualActions("delete-attribute", decoded,
                                                data_node);
            if (trs === undefined)
                return;

            for(var tr_ix = 0, tr; (tr = trs[tr_ix]) !== undefined; ++tr_ix) {
                pushItem({name: decoded, node: data_node}, tr);
            }
        }
    }
    else {
