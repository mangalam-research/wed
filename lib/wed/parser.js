...
(function () {
    this.start = function () {
        ...
        if (this._states.length === 0) { // create initial state
            ...
            require(["requirejs/text!" + this.schema], function(x) {
                var tree = validate.constructTree(x);
                me._states = [new State(0, tree.newWalker(), me.root, CONTENTS)];
    ...
    this._work = function () {
        ...
        while (true) {
            ...
            var walker = current_state.walker.clone();
            var element_to_parse = current_state.element_to_parse;
            var parsing_stage = current_state.parsing_stage;
            ...
            while (true) {
                ...
                case START_TAG:
                    ...
                    if (!$element_to_parse.hasClass("_phantom_wrap") && 
                        ...
                        event_result = walker.fireEvent(new validate.Event("enterStartTag", ename.ns, ename.name));
                        ...
                        for(var attr_ix = 0; attr_ix < attr_ix_lim; ++attr_ix) {
                            ...
                            event_result = walker.fireEvent(new validate.Event("attributeName", ename.ns, ename.name));
                            if (event_result !== true)
                                this.processTagEventResult(event_result, element_to_parse);
                            event_result = walker.fireEvent(new validate.Event("attributeValue", attr.value));
                            if (event_result !== true)
                                this.processTagEventResult(event_result, element_to_parse);
                        ...
                        // Leave the start tag.
                        event_result = walker.fireEvent(new validate.Event("leaveStartTag"));
                        if (event_result !== true)
                            this.processTagEventResult(event_result, element_to_parse);
                    }
                    ...
                    $element_to_parse.data("wed_start_state", new_state.number);
                ...
                case CONTENTS:
                    ...
                    while (node !== null) { 
                        ...
                        case Node.TEXT_NODE:
                            ...
                            // text is possible.
                            event_result = fireTextEventIfNeeded(node, walker);
                            if (event_result !== true) 
                                // XXX
                ...
                case END_TAG:
                    ...
                    if (!$element_to_parse.hasClass("_phantom_wrap") && 
                        ...
                        var name = element_to_parse.className.split(" ", 1)[0];
                        var ename = this.resolver.resolveName(name);
                        event_result = walker.fireEvent(new validate.Event("endTag", ename.ns, ename.name));
                        if (event_result !== true)
                            this.processTagEventResult(event_result, element_to_parse);
                    }
                    ...
                    if (element_to_parse !== this.root) {
                        var start_state = 
                            this._states[$(element_to_parse).data("wed_start_state")];

                        this._part_done = start_state.part_done += portion;
                        portion = start_state.portion;
                    }
                    ...
                    var new_state = new State(
                        ...
                        portion);
                    this._states.push(new_state);
                    $(original_element).data("wed_end_state", new_state.number);
                    continue state_change;
    ...
    this.restartAt = function (element) {
        // It might seem strange to issue start() before we initialize
        // the values but due to the single-threaded nature of JS the
        // worker code won't actually run before we're done here.
        this.start();
        this._states.forEach(function (state) {
            $(state.element_to_parse).removeData(["wed_end_state", "wed_start_state"]);
        });
        if (this._states.length > 0) {
            this._states = [this._states[0]];
            this._states[0].part_done = 0;
        }
        this._part_done = 0;
        this._last_done_shown = 0;
    };
    ...
    this.possibleAt = function (container, index) {
        if (container === this.root)
            // empty document
            return this._states[0].walker.possible();

        var evs = [];
        switch(container.nodeType) {
        case Node.TEXT_NODE:
            // We need the previous sibling which is not text.
            var $prev = $(container).prevAll("._real, ._phantom_wrap").not("._text").first();
            var walker = 
                (this._states[
                    ($prev.length > 0) ? $prev.data("wed_end_state") :
                        $(container.parentNode).data("wed_start_state")]).walker.clone();
        ...
        case Node.ELEMENT_NODE:
            ...
            // We need the previous sibling which is not text.
            var $prev = $(node).prevAll("._real, ._phantom_wrap").not("._text").first();
            
            var walker =  
                (this._states[($prev.length > 0) ?
                              $prev.data("wed_end_state") :
                              $(container).data("wed_start_state")]).walker;

            if ($prev.length > 0) {
                // If there is any text node between prev and the node of interest,
                // we must fire a text event.
                var prev = $prev.get(0); 
                var next_node = prev.nextSibling;
                var found = false;
                while(!found && next_node !== node) {
                    found = (next_node.nodeType === Node.TEXT_NODE || 
                              $(next_node).hasClass("_text"));
                    next_node = next_node.nextSibling;
                }

                if (found) { // There *is* text.
                    walker = walker.clone();
                    if (fireTextEventIfNeeded(next_node, walker) === true)
                        evs = walker.possible();
                }
                else 
                    evs = walker.possible();
...
function State(number, walker, element_to_parse, parsing_stage, 
               previous_child, part_done, portion) {
    this.number = number;
    this.walker = walker;
    this.element_to_parse = element_to_parse;
    this.parsing_stage = parsing_stage || START_TAG;
