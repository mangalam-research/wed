...
DOMUndo.prototype.toString = function () {
    function dump ($it) {
        return "(" + $it.length + ") " +
            ($it.clone().wrap('<div>').parent().html() || $it.text());
    }
    return [this._desc, "\n",
            " Parent: ",  dump(this._$parent), "\n",
            " Added: ", dump(this._$added), "\n",
            " Removed: ", dump(this._$removed), "\n",
            " Prev: ", dump(this._$prev), "\n",
            " Next: ", dump(this._$next), "\n"].join("");
};
...
TextNodeUndo.prototype.toString = function () {
    return [this._desc, "\n",
