...
/**
 * <p>Aggresively installs an onerror handler. It will install it both
 ...
 * iframe on Chrome 28.0.1500.95, I've noticed that the onerror
 ...
 * configration option for this module in your RequireJS setup.</p>
 *
 * <p>Note that it is extremely unlikely that wed would play well with
 * any onerror handler set by another software if this handler tries
...
function installOnError(win) {
    ...
    function onerror(msg, url, linenumber) {
        ...
        function terminate() {
            ...
            $("body").append($modal);
            $modal.find(".modal-body").contents().replaceWith(messages.join(""));
