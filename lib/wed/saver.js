 ...
 *
 * @class
 * @param {String} url The url where to POST save requests.
...
Saver.prototype.save = function () {
    ...
    function success (data) {
        ...
        try {
            ...
            if (msgs.save_corrupted)
                throw new Error(
                    "The server determined that the data sent to the " +
                        "server was corrupted somehow. This has been logged " +
                        "on the server. Please contact technical support " +
                        "before trying to edit again.");

            if (msgs.save_validation_error) {
                this._emit("failed", msgs.save_validation_error);
                return;
            }
