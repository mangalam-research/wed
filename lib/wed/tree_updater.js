...
define(/** @lends module:tree_updater */ function f(require, exports) {
  "use strict";
  ...
  TreeUpdater.prototype._emit = function _emit() {
   ...
   * ``what`` is not an element or text node.
   */
  TreeUpdater.prototype.insertAt = function insertAt(loc, offset, what) {
    var parent;
    var index;
   ...
   * if the call would merely split a text node in two.
   */
  TreeUpdater.prototype.splitAt = function splitAt(top, loc, index) {
    var node;
    if (loc instanceof DLoc) {
      node = loc.node;
      index = loc.offset;
   ...
   * second half of the split.
   */
  TreeUpdater.prototype._splitAt = function _splitAt(top, node, index) {
   ...
   */
  TreeUpdater.prototype.insertBefore = function insertBefore(parent, to_insert,
                                                             before_this) {
   ...
   * @throws {Error} If ``node`` is not an element or text Node type.
   */
  TreeUpdater.prototype.insertText = function insertText(loc, index, text) {
    var node;
    if (loc instanceof DLoc) {
      text = index;
      node = loc.node;
   ...
   * @throws {Error} If ``node`` is not a text Node type.
   */
  TreeUpdater.prototype.deleteText = function deleteText(loc, index, length) {
    var node;
    if (loc instanceof DLoc) {
      length = index;
      node = loc.node;
   ...
   * @throws {Error} If the node to insert is undefined or null.
   */
  TreeUpdater.prototype.insertIntoText = function insertIntoText(loc, index,
                                                                 node) {
    var parent;
    if (loc instanceof DLoc) {
      node = index;
   ...
   * @throws {Error} If ``node`` is a document fragment Node type.
   */
  TreeUpdater.prototype.insertNodeAt = function insertNodeAt(loc, index, node) {
    var parent;
    if (loc instanceof DLoc) {
      node = index;
      index = loc.offset;
     ...
     * @property {Node} node
     */
    this._emit("insertNodeAt", { parent: parent,
      index: index,
      node: node });
  };
   ...
   */
  TreeUpdater.prototype.setTextNode = function setTextNode(node, value) {
    if (node.nodeType !== Node.TEXT_NODE) {
   ...
   * @throws {Error} If called on a non-text Node type.
   */
  TreeUpdater.prototype.setTextNodeValue = function setTextNodeValue(node,
                                                                     value) {
    if (node.nodeType !== Node.TEXT_NODE) {
   ...
   */
  TreeUpdater.prototype.removeNode = function removeNode(node) {
   ...
   * no node to remove.
   */
  TreeUpdater.prototype.removeNodeNF = function removeNodeNF(node) {
    if (!node) {
      return undefined;
    }
   ...
   */
  TreeUpdater.prototype.removeNodes = function removeNodes(nodes) {
    if (!nodes.length) {
      return undefined;
    }
   ...
   */
  TreeUpdater.prototype.cut = function cut(start, end) {
   ...
   */
  TreeUpdater.prototype.mergeTextNodes = function mergeTextNodes(node) {
   ...
   * no node to remove.
   */
  TreeUpdater.prototype.mergeTextNodesNF = function mergeTextNodesNF(node) {
    if (!node) {
      return undefined;
    }
   ...
   * @emits module:tree_updater~TreeUpdater#change
   */
  TreeUpdater.prototype.deleteNode = function deleteNode(node) {
    /**
   ...
   * @emits module:tree_updater~TreeUpdater#change
   */
  TreeUpdater.prototype.setAttribute = function setAttribute(node, attribute,
                                                             value) {
   ...
   * @emits module:tree_updater~TreeUpdater#change
   */
  TreeUpdater.prototype.setAttributeNS = function setAttributeNS(node, ns,
                                                                 attribute,
                                                                 value) {
    // Normalize to null.
    if (value === undefined) {
    ...
    this._emit("setAttributeNS", { node: node,
      ns: ns,
      attribute: attribute,
      old_value: old_value,
      new_value: value });
   ...
   */
  TreeUpdater.prototype.nodeToPath = function nodeToPath(node) {
   ...
   */
  TreeUpdater.prototype.pathToNode = function pathToNode(path) {
