 ...
 * between "foo" and "cd".
 */
function insertIntoText(text_node, index, node) {
    if (!node)
        throw new Error("must pass an actual node to insert");
    return _insertIntoText(text_node, index, node);
}
 ...
 * between "foo" and "cd".
 */
function _insertIntoText(text_node, index, node, clean) {
    ...
    var search_node, parent, prev, next;
    ...
    if (clean && (!node || (is_fragment && node.childNodes.length === 0)))
    {
        ...
        // A document fragment will disappear upon insertion, so
        // the node we search for is its first child.
        search_node = (is_fragment) ? node.firstChild : node;
        parent = text_node.parentNode;

        parent.insertBefore(node, text_node);
        // The DOM sequence is now:
        // ...
        // node
        // text_node
        // ...

        prev = text_node.previousSibling;
        start_caret = [parent,
                       Array.prototype.indexOf.call(parent.childNodes,
                                                    search_node)];
        if (clean && prev.nodeType === Node.TEXT_NODE) {
            parent.removeChild(prev);
            text_node.nodeValue = prev.nodeValue + text_node.nodeValue;
            end_caret = [text_node, prev.nodeValue.length];
            // We've just removed prev so the start_caret we have set
            // is no good anymore.
            if (prev === search_node)
                start_caret = [text_node, 0];
        }
        else
            end_caret = [text_node, 0];
    }
    else if (clean && index === text_node.nodeValue.length) {
        // A document fragment will disappear upon insertion, so
        // the node we search for is its last child.
        search_node = is_fragment ? node.lastChild : node;
        parent = text_node.parentNode;

        // This inserts it after the text node.
        text_node.parentNode.insertBefore(node, text_node.nextSibling);
        // The DOM sequence is now:
        // ...
        // text_node
        // node
        ...
        start_caret = [text_node, text_node.nodeValue.length];
        end_caret = [parent, Array.prototype.indexOf.call(parent.childNodes,
                                                          search_node) + 1];

        if (clean && next.nodeType === Node.TEXT_NODE) {
            parent.removeChild(next);
            text_node.nodeValue += next.nodeValue;
            if (next === search_node)
                // The lastChild of the fragment was its only node, which is
                // now merged with text_node.
                end_caret = [text_node, text_node.nodeValue.length];
        }
    }
    else {
...
/**
 * Removes the node. Mainly for use with {@link
 * module:domutil~makeCutFunction makeCutFunction}.
 ...
 * text, does nothing. Mainly for use with @{link
 * module:domutil~makeCutFunction}.
 *
 ...
 * @returns {Array} A list of nodes. The cut contents.
 */
