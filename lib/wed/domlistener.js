 ...
 *
 *    <dd>handler($root, $parent, $previous_sibling, $next_sibling,
 *  $element)</dd>
 *
 *  <dt>included-element, excluded-element:</dt>
 *
 *    <dd>handler($root, $element)</dd>
 *
 *  <dt>children-changed:</dt>
 *
 *    <dd>handler($root, $added, $removed, $element)</dd>
 *
 * <dt>text-changed:</dt>
 *
 *    <dd>handler($root, $element)</dd>
 ...
 * </dl>
 *
 * <p>There is no reason to provide <code>$parent, $previous_sibling,
 * $next_sibling</code> for an added-element event but having the same
 ...
 * the parent that the element had <strong>before</strong> it was
 * removed. The $element parameter is a single element.</p>
 *
 * <p>Note that the meaning of the <code>selector</code> parameter for
 ...
 * which itself is always a text node.</p>
 *
 * <p>Another thing to keep in mind is that a text-changed event is
 * not generated when Node objects of type TEXT_NODE are added or
 * removed. They trigger children-changed events.</p>
 *
...
oop.implement(Listener,
/** @lends module:domlistener~Listener# */
{
    /**
     * Start listening to changes on the root passed when the object
     * was constructed.
     */
     ...
     *
     * @param {String|Array.<String>} event_types Either a string
     * naming the event this handler will process or an array of
     * strings if multiple types of events are to be handled.
     * @param selector Can be anything that jQuery accepts as a selector.
     * @param {Function} handler The handler to be called by this
     * listener when the events specified in <code>event_types</code>
     * occur.
     */
    addHandler: function (event_types, selector, handler) {
    ...
    _onMutation: function (mutations, observer) {
        for(var mut_ix = 0, mut_ix_limit = mutations.length;
            ...
            if (mut.type === "childList") {
                ...
                for(node_ix = 0; !!(node = mut.removedNodes[node_ix]);
                    node_ix++)
                    if (node.nodeType !== Node.TEXT_NODE)
    ...
    _fireAddRemHandlers: function (name, mutation, nodes, $target) {
    ...
    _fireIncExcHandlers: function (name, nodes) {
        var pairs = this._event_handlers[name];
