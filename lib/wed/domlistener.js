 ...
 */
define(/** @lends module:domlistener */ function f(require, exports) {
  "use strict";
   ...
   */
  Listener.prototype.addHandler = function addHandler(event_types,
                                                      selector, handler) {
    if (!(event_types instanceof Array)) {
      event_types = [event_types];
    }

    for (var ev_ix = 0; ev_ix < event_types.length; ev_ix++) {
      var event_type = event_types[ev_ix];
   ...
   */
  Listener.prototype.trigger = function trigger(name) {
   ...
   * @private
   */
  Listener.prototype._processTriggers = function _processTriggers() {
    ...
    while (keys.length > 0) {
      ...
      for (var key_ix = 0; key_ix < keys.length; ++key_ix) {
        var key = keys[key_ix];
        var handlers = trigger_map[key];
        if (handlers !== undefined) {
          for (var handler_ix = 0; handler_ix < handlers.length; ++handler_ix) {
            this._callHandler(handlers[handler_ix]);
          }
        }
      }
   ...
   */
  Listener.prototype._callHandler = function _callHandler() {
  ...
  Listener.prototype._dumpMutation = function _dumpMutation(mut) {
