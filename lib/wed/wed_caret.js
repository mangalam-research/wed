...
define(/** @lends module:wed */function f(require) {
  ...
  EditorP.dumpCaretInfo = function dumpCaretInfo() {
  ...
  EditorP.caretPositionRight = function caretPositionRight() {
    return this.positionRight(this._sel_focus);
  };

  EditorP.positionRight = function positionRight(pos) {
    ...
    if (closest_ph) {
      pos = pos.make(closest_ph.parentNode,
                     indexOf(closest_ph.parentNode.childNodes, closest_ph) + 1);
    }
    ...
    while (true) { // eslint-disable-line no-constant-condition
      ...
      var nextCaret = domutil.nextCaretPosition(pos.toArray(), container,
                                                false);
      if (!nextCaret) {
        pos = null;
        break;
      ...
      if (closest_gui) {
        ...
        if (this.attributes === "edit" && start_label) {
          ...
          if (closest(node, "._element_name, ._attribute", root)) {
            ...
            var next_attr = _.find(
              closest_gui.getElementsByClassName("_attribute"), nextAttrFilter);

            if (next_attr) {
              // There is a next attribute: move to it.
              var val = domutil.childByClass(next_attr, "_attribute_value");
        ...
        pos = pos.make(
          ...
          (start_label || closestByClass(node, "_label", closest_gui)) ?
            node.getElementsByClassName("_element_name")[0] : closest_gui, 0);
        // ... stop here.
        break;
      }
  ...
  EditorP.caretPositionLeft = function caretPositionLeft() {
    return this.positionLeft(this._sel_focus);
  };

  EditorP.positionLeft = function positionLeft(pos) {
    if (pos === undefined || pos === null) {
      return undefined;
    } // nothing to be done
    ...
    while (true) { // eslint-disable-line no-constant-condition
      ...
      var was_in_name = el_name && (pos.node === el_name) && (pos.offset === 0);
      ...
      var prevCaret = domutil.prevCaretPosition(pos.toArray(), container,
                                                false);
      if (!prevCaret) {
        pos = null;
        break;
      ...
      if (closest_gui) {
        ...
        if (this.attributes === "edit" && start_label && !was_in_name) {
          ...
          if (!attr) {
            el_name = closestByClass(node, "_element_name", closest_gui);
          ...
          if (prev_attr && !prev_attr.classList.contains("_attribute")) {
            prev_attr = null;
          }

          if (prev_attr) {
            // There is a previous attribute: move to it.
            var val = domutil.childByClass(prev_attr, "_attribute_value");
            offset = 0;
            if (val.lastChild) {
              val = val.lastChild;
              offset = val.length;
              if (val.classList && val.classList.contains("_placeholder")) {
                offset = 0;
              }
            }
      ...
      if (node.nodeType === Node.ELEMENT_NODE) {
        ...
        if (next_node &&
            ...
            ((prev_node &&
             ...
             next_node.matches("._gui.__start_label, .wed-validation-error"))) {
          continue;
        } // can't stop here
  ...
  EditorP.moveCaretRight = function moveCaretRight() {
  ...
  EditorP.moveCaretLeft = function moveCaretLeft() {
  ...
  EditorP.caretPositionDown = function caretPositionDown() {
    return this.positionDown(this._sel_focus);
  };

  EditorP.positionDown = function positionDown(pos) {
    if (pos == null) {
      return undefined;
    }

    // Search for the next line.
    var initialCaret = wed_util.boundaryXY(pos);
    var next = initialCaret;
    while (pos && initialCaret.bottom > next.top) {
      pos = this.positionRight(pos);
      if (pos == null) {
        return undefined;
      }
      next = wed_util.boundaryXY(pos);
    }
    ...
    // pos is now at the start of the next line. We need to find the position
  ...
  EditorP.moveCaretDown = function moveCaretDown() {
    var pos = this.caretPositionDown();
    if (pos) {
      this.setGUICaret(pos);
    }
  };

  EditorP.caretPositionUp = function caretPositionUp() {
    return this.positionUp(this._sel_focus);
  };

  EditorP.positionUp = function positionUp(pos) {
    if (pos == null) {
      return undefined;
    }

    // Search for the previous line.
    var initialBoundary = wed_util.boundaryXY(pos);
    var prev = initialBoundary;
    while (pos && initialBoundary.top < prev.bottom) {
      pos = this.positionLeft(pos);
      if (pos == null) {
        return undefined;
      }
      prev = wed_util.boundaryXY(pos);
    }
  ...
  EditorP.moveCaretUp = function moveCaretUp() {
    var pos = this.caretPositionUp();
    if (pos) {
      this.setGUICaret(pos);
    }
  };
   ...
   */
  EditorP.setGUICaret = function setGUICaret(loc, offset) {
   ...
   * the possible values.
   */
  EditorP._setGUICaret = function _setGUICaret(loc, offset, op) {
    var node;
    ...
    if (node.classList && node.classList.contains("_attribute_value")) {
  ...
  EditorP._focusInputField = function _focusInputField() {
  ...
  EditorP._blur = log.wrap(function _blur() {
    if (!this._sel_focus) {
      return;
    }
  ...
  EditorP._focus = log.wrap(function _focus() {
  ...
  EditorP._inhibitFakeCaret = function _inhibitFakeCaret() {
    this._inhibited_fake_caret++;
  };

  EditorP._uninhibitFakeCaret = function _uninhibitFakeCaret() {
    this._inhibited_fake_caret--;
  ...
  EditorP._refreshFakeCaret = function _refreshFakeCaret() {
    ...
    position = {
      top: position.top - gr_position.top,
   ...
   * fake caret relative to the GUI root.
   */
  EditorP._fakeCaretPositionFromGUIRoot =
    function _fakeCaretPositionFromGUIRoot() {
      // This function may be called when the caret layer is invisible. So we
      // can't rely on offset. Fortunately, the CSS values are what we want,
      // so...
      var element = this._fake_caret;
      var pos = {
      ...
      pos.left += this._scroller.scrollLeft;
      pos.top += this._scroller.scrollTop;
   ...
   */
  EditorP.getGUICaret = function getGUICaret(raw) {
    // Caret is unset
  ...
  EditorP._normalizeCaret = function _normalizeCaret(loc) {
    if (!loc) {
      return loc;
    }
  ...
  EditorP.fromDataLocation = function fromDataLocation(node, offset) {
    ...
    if (node.nodeType === Node.ELEMENT_NODE) {
      ...
      if (new_offset <= first_index) {
        ...
        var second_index = pair[1] ? indexOf(node.childNodes, pair[1]) :
   ...
   * parameter.
   * @param {Boolean} [approximate=false] Some GUI locations do not
   * correspond to data locations. Like if the location is in a gui
   * element or phantom text. By default, this method will return
   * undefined in such case. If this parameter is true, then this method
   ...
   * not correspond to a location in the data tree.
   */
  EditorP.toDataLocation = function toDataLocation(loc, offset, approximate) {
    var node;
    var root;
    if (loc instanceof DLoc) {
      approximate = offset;
    ...
    if (!closestByClass(node, "_attribute_value", root)) {
      ...
      var wrap = closestByClass(check, "_phantom_wrap", root);
      if (wrap) {
        // We are in a phantom wrap. Set position to the real
        // element being wrapped. This is not considered to be an
        // "approximation" because _phantom_wrap elements are
        // considered visual parts of the real element.
        var real = wrap.getElementsByClassName("_real")[0];
        node = real.parentNode;
        offset = indexOf(node.childNodes, real);
      }
      else {
        while (check) {
    ...
    var normalized = this._normalizeCaret(makeDLoc(this.gui_root, node, offset));
    ...
    if (child.nodeType !== Node.TEXT_NODE &&
      ...
      return makeDLoc(this.data_root, data_node.parentNode,
                      indexOf(data_node.parentNode.childNodes, data_node) + 1);
    }
  ...
  EditorP.getDataCaret = function getDataCaret(approximate) {
   ...
   * for a text editing operation.
   */
  EditorP.setDataCaret = function setDataCaret(loc, offset, text_edit) {
    if (loc instanceof DLoc) {
      text_edit = offset;
    }
    else {
   ...
   */
  EditorP._caretChange = log.wrap(function _caretChange(op) {
    var text_edit = false;
    var focus = false;
    ...
    var pos = this._fakeCaretPositionFromGUIRoot();
    var $fake = $(fake);
    if (!focus) {
      this.scrollIntoView(pos.left, pos.top, pos.left + $fake.outerWidth(),
   ...
   */
  EditorP.pushSelection = function pushSelection() {
   ...
   */
  EditorP.popSelection = function popSelection() {
   ...
   */
  EditorP.popSelectionAndDiscard = function popSelectionAndDiscard() {
    this._selection_stack.pop();
  };
   ...
   */
  EditorP._restoreCaretAndSelection = function _restoreCaretAndSelection(focus) {
    if (this._sel_anchor &&
  ...
  EditorP.clearSelection = function clearSelection() {
    this._sel_anchor = undefined;
    this._sel_focus = undefined;
    this._refreshFakeCaret();
  ...
  EditorP._getDOMSelection = function _getDOMSelection() {
   ...
   */
  EditorP._clearDOMSelection = function _clearDOMSelection(dont_focus) {
  ...
  EditorP._getDOMSelectionRange = function _getDOMSelectionRange() {
  ...
  EditorP.getSelectionRange = function getSelectionRange() {
    return this._sel_anchor ? this._sel_anchor.makeRange(this._sel_focus).range
      : undefined;
  };

  EditorP.setSelectionRange = function setSelectionRange(range, reverse) {
    var start = makeDLoc(this.gui_root, range.startContainer, range.startOffset);
  ...
  EditorP._normalizeCaretToEditableRange =
    function _normalizeCaretToEditableRange(container, offset) {
   ...
   * @private
   */
  EditorP._setDOMSelectionRange = function _setDOMSelectionRange(range,
                                                                 reverse) {
    if (range.collapsed) {
      this._clearDOMSelection();
      return;
  ...
  EditorP.getDataSelectionRange = function getDataSelectionRange() {
  ...
  EditorP.setDataSelectionRange = function setDataSelectionRange(range) {
   ...
   */
  EditorP._caretPositionOnScreen = function _caretPositionOnScreen() {
    if (!this._sel_focus) {
      return undefined;
    }
    ...
    var node = this._sel_focus.node;
    if (node.classList && node.classList.contains("_gui")) {
