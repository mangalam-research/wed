...
define(/** @lends module:wed */function f(require) {
  ...
  EditorP.setCaret = function setCaret(node, offset, options) {
    this.caretManager.setCaret(node, offset, options);
  };
   ...
   * the possible values.
   */
  EditorP._setGUICaret = function _setGUICaret(loc, op) {
    this.caretManager._setGUICaret(loc, op);
  };

  EditorP.getDataCaret = function getDataCaret(approximate) {
    return this.caretManager.getDataCaret(approximate);
  };

  EditorP.getGUICaret = function getGUICaret(raw) {
    return this.caretManager.getGUICaret(raw);
  };
    ...
    if (boundary) {
      ...
      if (((node_type === Node.ELEMENT_NODE) &&
        ...
        case Node.TEXT_NODE:
          ...
          // editing standpoint.
          var right = this.caretManager.positionRight(boundary);
          var left = this.caretManager.positionLeft(boundary.make(node,
                                                                  offset + 1));
          if (right && !left) {
            boundary = right;
          }
    ...
    if (caret) {
      ...
      if (focus_node.nodeType === Node.ELEMENT_NODE) {
        ...
        if (ph && !ph.classList.contains("_dying")) {
          this._setGUICaret(caret.make(ph, 0), options);
          return;
        }
      }
    }
  ...
  EditorP._restoreCaretAndSelection = function _restoreCaretAndSelection(focus) {
    this.caretManager._restoreCaretAndSelection(focus);
  };

  EditorP.setSelectionRange = function setSelectionRange(range, reverse) {
    this.caretManager.setSelectionRange(range, reverse);
  };
  ...
  EditorP.getDataSelectionRange = function getDataSelectionRange() {
    return this.caretManager.getDataSelectionRange();
  };

  EditorP.setDataSelectionRange = function setDataSelectionRange(range) {
    this.caretManager.setDataSelectionRange(range);
  };
  ...
  EditorP._caretPositionOnScreen = function _caretPositionOnScreen() {
    return this.caretManager._caretPositionOnScreen();
  };
});
