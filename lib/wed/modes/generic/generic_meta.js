 ...
 */
define(/** @lends module:modes/generic/generic_meta */function f(require,
                                                                 exports) {
  "use strict";
   ...
   * @emits module:lib/pubsub#WED_MODES_GENERIC_META_READY
   */
  Meta.prototype._ready = function _ready() {
   ...
   * @private
   */
  Meta.prototype._resolveOptions = function _resolveOptions() {
    var options = this._options;
    var resolved = $.extend(true, {}, options);
    if (options && options.metadata) {
      this._runtime.resolve(options.metadata).then(function success(data) {
   ...
   * @private
   */
  Meta.prototype._afterResolved = function _afterResolved() {
    ...
    if (this._metadata) {
      ...
      elements.forEach(function each(el) {
        ...
        var elNs = el.ns || "http://www.tei-c.org/ns/1.0";
        var elPrefix = me._reverse_mapping[elNs];
        if (elPrefix === undefined) {
          throw new Error("undefined namespace: " + elNs);
        }
        var name = elPrefix === "" ? el.name : (elPrefix + ":" + el.name);
  ...
  Meta.prototype.isInline = function isInline(node) {
    return false;
  };
   ...
   */
  Meta.prototype.getAdditionalClasses = function getAdditionalClasses(node) {
    var ret = [];
   ...
   */
  Meta.prototype.getNamespaceMappings = function getNamespaceMappings() {
   ...
   * yet.
   */
  Meta.prototype.shortDescriptionFor = function shortDescriptionFor(name) {
   ...
   */
  Meta.prototype.documentationLinkFor = function documentationLinkFor(name) {
    if (!this._metadata) {
      return undefined;
    }
