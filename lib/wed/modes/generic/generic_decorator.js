 ...
 * is applied.
 */
function GenericDecorator(mode, meta) {
    ...
    this._domlistener.addHandler(
        "children-changed",
        util.classFromOriginalName("*"),
        function ($root, $added, $removed, $previous_sibling, $next_sibling, $element) {
    ...
    this._domlistener.addHandler(
        ...
        function ($root, $element) {
            this.elementDecorator(
                $root, $element.parent(),
                util.eventHandler(this._contextMenuHandlerAtStart.bind(this)),
                util.eventHandler(this._contextMenuHandlerAtEnd.bind(this)));
        }.bind(this));
...
GenericDecorator.prototype._contextMenuHandler = function (at_start, ev, jQthis) {
    ...
    if (node.parentNode !== this._editor.root) {
        ...
        var tree_caret = this._editor.toTreeCaret(parent, index);
        var handler = function(e) {
            this._editor._fireTransformation(e, tree_caret);
        }.bind(this);
        this._editor.validator.possibleAt(
            tree_caret[0], tree_caret[1]).forEach(function (ev) {
                ...
                for(var tr_ix = 0, tr; (tr = trs[tr_ix]) !== undefined;
                    ...
                    $a.click({'tr': tr, 'element_name': unresolved }, handler);
        ...
        this._editor.displayContextMenu(ev.pageX, ev.pageY, menu_items);
        return false;
    }
...
GenericDecorator.prototype._decorateContent = function ($root, $element) {
