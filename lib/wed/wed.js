...
Editor.prototype.init = function (widget, options) {
    ...
    this._gui_updater = new GUIUpdater(
        this.$gui_root.get(0), this.$data_root.get(0), this.data_updater);
 ...
 <div id="sb-errors" class="accordion-body collapse in">\
  <div class="accordion-inner">\
   <ul id="sb-errorlist" class="nav nav-list">\
    ...
    this.menu_layer = $("<div id='menu-layer'>").get(0);
    ...
    this._cut_modal = this.makeModal();
    this._cut_modal.setTitle("Invalid deletion");
    this._cut_modal.setBody(
        ...
        smaller sections.<p>");
    this._cut_modal.addButton("Ok", true);
    ...
    var glyphicons_css = require.toUrl("glyphicons/css/bootstrap-glyphicons.css");
    this.$frame.children("head").prepend('<link rel="stylesheet" href="' + glyphicons_css + '" type="text/css" />');
...
Editor.prototype._postInitialize = function  () {
    ...
    this.data_domlistener = new domlistener.Listener(this.$data_root);
    this.data_domlistener.startListening();
    ...
    // We have to listed to included-element rather than
    // added-element because the data_domlistener may add entire
    // trees.
    this.gui_domlistener.addHandler(
        "included-element",
        "._real, ._phantom_wrap",
    ...
    this.data_domlistener.addHandler(
        "children-changed",
        "._real, ._phantom_wrap",
        function ($root, $added, $removed, $prev, $next, $target) {
        ...
        // Narrow it to the elements we care about.
        var $narrow_added =
            $added.filter("._real, ._phantom_wrap, ._placeholder").
            add($added.filter(jqutil.textFilter));
        var $narrow_removed =
            $removed.filter("._real, ._phantom_wrap, ._placeholder").
            add($removed.filter(jqutil.textFilter));
        if ($narrow_added.length + $narrow_removed.length === 0)
            return;
        ...
        this._undo.record(new wundo.DOMUndo(
            this, $target, $narrow_added, $narrow_removed, $prev, $next));
    }.bind(this));
    ...
    this.data_domlistener.addHandler(
        ...
        if (this._suppress_undo_recording)
            return;
        // Narrow it to the elements we care about.
        this._undo.record(new wundo.TextNodeUndo(this, $element, old_value));
    }.bind(this));
    ...
    $(this.my_window).on('resize', this._resizeHandler.bind(this));
...
Editor.prototype._syncDisplay = function () {
    this.data_domlistener.processImmediately();
    this.gui_domlistener.processImmediately();
};
 ...
 * transformation.
 * @param {Array} new_caret_position A new caret position in the data
 * tree.
 */
Editor.prototype.fireTransformation = function(tr, node, element_name,
    ...
    if (new_caret_position)
        this.setDataCaret(new_caret_position);
    ...
    this._undo.startGroup(
        new wundo.UndoGroup("Undo " + tr.getDescriptionFor(element_name),
                            this));
    if (node !== undefined) {
...
Editor.prototype._fireTransformation = function (e, new_caret_position) {
    this.fireTransformation(e.data.tr, e.data.node, e.data.element_name,
                            undefined, new_caret_position);
};
...
Editor.prototype._contextMenuHandler = function (e, jQthis) {
    ...
    if (!$node_of_interest.hasClass("_phantom") &&
        ...
        this.validator.possibleAt(
            ...
            offset).forEach(function (ev) {
                ...
                var trs = this.mode.getTagTransformations(
        ...
        if ($node_of_interest.get(0) !== this.gui_root.childNodes[0]) {
            ...
            var trs = this.mode.getTagTransformations("delete-parent", orig);
            if (trs !== undefined) {
    ...
    if (sep_for !== undefined) {
        ...
        trs.forEach(function (tr) {
            var $a = $("<a tabindex='0' href='#'>" +
                       tr.getDescriptionFor(sep_for) + "</a>");
            $a.click({'tr': tr, 'node': $transformation_node.get(0),
...
Editor.prototype._cutHandler = function(e, jQthis) {
    ...
    if (transformation.isWellFormedRange(range)) {
        ...
        this.my_window.setTimeout(function () {
            this.fireTransformation(cut_tr, caret[0], undefined,
                                    {e: e, jQthis: jQthis,
...
function cut(editor, node, element_name, data) {
    ...
    if (start_container.nodeType === Node.TEXT_NODE) {
        ...
        var start_container_offset = Array.prototype.indexOf.call(
            parent.childNodes, start_container);

        domutil.deleteText(
            start_container,
            start_offset,
            (same_container ? end_offset : start_container.nodeValue.length) -
                start_offset);
    ...
    if (end_container.nodeType === Node.TEXT_NODE) {
        ...
        var end_container_offset = Array.prototype.indexOf.call(
            parent.childNodes, end_container);

        domutil.deleteText(end_container, 0, end_offset);
    ...
    while (end_offset >= start_offset)
        end_container.removeChild(end_container.childNodes[end_offset--]);
    ...
    editor.tree_updater.mergeTextNodes(end_container.childNodes[end_offset]);
}

var cut_tr = new transformation.Transformation("Cut", cut);
            ...
            if (errors) {
                this._paste_modal.modal(function () {
                    if (this._paste_modal.getClickedAsText() === "Yes") {
                        ...
                        this.fireTransformation(paste_tr, caret[0], undefined,
                                                {$data: $data, e: e,
                                                 jQthis: jQthis});
                    }
                }.bind(this));
...
var paste_tr = new transformation.Transformation("Paste", paste);
...
Editor.prototype._globalKeydownHandler = function (wed_event, e, jQthis) {
    ...
    if (key_constants.DELETE.matchesEvent(e)) {
        ...
        if (!$(domutil.nextCaretPosition(this._raw_caret, this.gui_root,
                                         true)[0])
    ...
    if (key_constants.BACKSPACE.matchesEvent(e)) {
        ...
        if (!$(domutil.prevCaretPosition(this._raw_caret, this.gui_root,
                                         true)[0])
            .is("._phantom, ._phantom_wrap")) {
...
Editor.prototype.destroy = function () {
    ...
    try {
        if (this.data_domlistener !== undefined)
            this.data_domlistener.stopListening();
    } catch(ex) { recorded = ex; }
