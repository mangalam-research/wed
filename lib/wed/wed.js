...
Editor.prototype.pushSelection = function () {
    ...
    var to_push;
    if (this._sel_anchor)
        to_push = ['anchor', this._sel_anchor, this._sel_focus];
    else if (sel.rangeCount > 0)
        to_push = ['rangy', rangy.saveSelection(this.my_window)];
    else
        to_push = ['caret', this._getRawCaret()];
    this._selection_stack.push(to_push);
};

Editor.prototype.popSelection = function () {
    var it = this._selection_stack.pop();
    if (it[0] === 'caret')
        this.setCaret(it[1]);  // setCaret() will call _caretChangeEmitter.
    else if (it[0] === 'anchor') {
        this._sel_anchor = it[1];
        this._sel_focus = it[2];
        var rr = domutil.rangeFromPoints(this._sel_anchor.node,
                                         this._sel_anchor.offset,
                                         this._sel_focus.node,
                                         this._sel_focus.offset);
        ...
        if (sel.rangeCount > 0 &&
            ...
            var range = sel.getRangeAt(0).nativeRange.cloneRange();
            this._sel_anchor = {node: range.startContainer,
                                offset: range.startOffset};
            this._sel_focus = {node: range.endContainer,
                               offset: range.endOffset};
            this.getDOMSelection().setSingleRange(range);
            this._setFakeCaretTo(sel.focusNode, sel.focusOffset);
            this._focusInputField();
            this._caretChangeEmitter();
        }
    }
