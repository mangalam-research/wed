...
Editor.prototype._postInitialize = log.wrap(function  () {
    ...
    this.domlistener.addHandler(
        ...
        if (this._refreshing_caret)
            return;
        this.domlistener.trigger("refresh-caret");
    }.bind(this));
...
Editor.prototype.dumpCaretInfo = function () {
    ...
    if (this._raw_caret)
        console.log("raw caret", this._raw_caret.node, this._raw_caret.offset);
    else
        console.log("no raw caret");
    ...
    if (this._fake_caret)
        ...
        console.log("no fake caret");

    if (data_caret)
        console.log("data closest real", $(data_caret.node).closest("._real"));

    if (this._raw_caret)
        console.log("gui closest real",
                    $(this._raw_caret.node).closest("._real"));

    domutil.dumpRange("DOM range: ", this.getDOMSelectionRange());
...
Editor.prototype._contextMenuHandler = function (e) {
    ...
    if (!$node_of_interest.hasClass("_phantom") &&
        ...
        if ($data_node_of_interest[0] !== this.data_root.firstChild) {
            var orig = getOriginalName($data_node_of_interest[0]);
            var trs = this.mode.getContextualActions(
                ["unwrap", "delete-parent"], orig,
                $data_node_of_interest[0], 0);
...
Editor.prototype.setGUICaret = function (loc, offset, force_fake, text_edit) {
    ...
    if (this._raw_caret &&
        this._raw_caret.node === node &&
        this._raw_caret.offset === offset)
        return;
    ...
    this._possibleCaretChange(undefined, text_edit);
};
...
Editor.prototype._blur = log.wrap(function (ev) {
    ...
    this._sel_anchor = undefined;
    this._sel_focus = undefined;
    this._possibleCaretChange(undefined, false, true);
});
...
Editor.prototype._refreshFakeCaret = function () {
    ...
    if (srange) {
        ...
        range.setEnd(
            ...
            srange.endOffset);
        // clearDOMSelection is to work around a problem in Rangy
...
Editor.prototype._globalKeydownHandler = log.wrap(function (wed_event, e) {
    ...
    if (e.which >= 33 /* page up */ && e.which <= 40 /* down arrow */) {
        ...
        var $raw_caret = $(raw_caret && raw_caret.node);
        if ($raw_caret.is("._label_clicked")) {
            $raw_caret.trigger("wed-context-menu", [e]);
            return terminate();
        }
    ...
    else if ($(raw_caret.node).is('._phantom, ._phantom_wrap'))
        return terminate();

    function handleRange() {
        var range = me.getDOMSelectionRange();
        if (range !== undefined) {
            if (!domutil.isWellFormedRange(range))
                return true;
    ...
    if (key_constants.DELETE.matchesEvent(e)) {
        ...
        if (!$(domutil.nextCaretPosition(raw_caret.toArray(), this.gui_root,
                                         true)[0])
    ...
    if (key_constants.BACKSPACE.matchesEvent(e)) {
        ...
        if (!$(domutil.prevCaretPosition(raw_caret.toArray(),
...
Editor.prototype._mousedownHandler = log.wrap(function(ev) {
    ...
    this.$gui_root.one('mouseup', this._possibleCaretChange.bind(this));
 ...
 */
Editor.prototype._possibleCaretChange = log.wrap(function (ev, text_edit,
                                                          dont_focus) {
    ...
    if (this._raw_caret === undefined ||
        ...
        this._caretChange(this._raw_caret, old_caret, text_edit);
    }
});

Editor.prototype._caretChange = function (caret, old_caret, text_edit) {
    if (!text_edit)
