...
Editor.prototype._postInitialize = log.wrap(function  () {
    ...
    this.$gui_root.on('keyup mouseup', this._caretChangeEmitter.bind(this));
...
Editor.prototype._mouseHandler = log.wrap(function (e) {
    if (e.type === "click") {
        this._undo.endAllGroups();
...
Editor.prototype._menuHandler = log.wrap(function (e) {
    var $target = $(e.currentTarget);
    function findFocus(going_down) {
        var $ret = $target.find('a:focus');
        if ($ret.length > 0)
            return $ret;
        ...
        if (going_down) {
            ...
            $($target.find('li').get().reverse()).each(function () {
                var $this = $(this);
    ...
    if (e.type === "menuselect") {
        ...
        case 13: // Enter
        case 32: // Space
            $focus = $target.find('a:focus');
            if ($focus.length > 0)
                $focus.click();
            else
                $target.data('wed-hover').click();
            this.dismissContextMenu();
            break;
        case 38: // Up
...
Editor.prototype._caretChangeEmitter = log.wrap(function (ev) {
    if (ev === undefined)
...
Editor.prototype.displayContextMenu = function (x, y, items) {
    ...
    $menu.on('keydown mouseleave mouseenter', this._menuHandler.bind(this));
