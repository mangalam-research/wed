...
Editor.prototype.fromDataCaret = function (node, offset) {
    ...
    if(node.nodeType === Node.ELEMENT_NODE) {
        ...
        if (ret[1] <= first_index)
            ...
            var second_index = _indexOf.call(ret[0].childNodes, pair[1]);
...
Editor.prototype._caretChangeEmitterTimeout = log.wrap(function (ev) {
    ...
    if (focus_node && focus_node.nodeType === Node.ELEMENT_NODE) {
        ...
        var ph = $(focus_node).children("._placeholder").get(0);
        if (ph) {
            this.setCaret(ph, 0);
            return;
        }
...
Editor.prototype._normalizeSelectionRange = function () {
    ...
    function _normalize(container, offset) {
        if(container.nodeType === Node.ELEMENT_NODE) {
            ...
            var first_index = _indexOf.call(container.childNodes, pair[0]);
            if (offset <= first_index)
                offset = first_index + 1;
            else {
                var second_index = _indexOf.call(container.childNodes, pair[1]);
