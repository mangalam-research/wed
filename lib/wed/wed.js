...
Editor.prototype.scrollIntoView = function (left, top, right, bottom) {
    // Bring the gui_root into view if the browser window has scroll
    // bars.
    this.gui_root.scrollIntoView();

    // Then adjust the gui_root.
    ...
    if (top < vtop || bottom > vbottom)
        // Not already in view.
        this.$gui_root.scrollTop(top < vtop ? top : bottom - vheight);
    ...
    if (left < vleft || right > vright)
        // Not already in view.
        this.$gui_root.scrollLeft(left < vleft ? left : right - vwidth);
};
