...
Editor.prototype._postInitialize = log.wrap(function  () {
    ...
    this.$gui_root.on('compositionstart compositionend',
                      this._compositionHandler.bind(this));
    this.$gui_root.on('input', this._inputHandler.bind(this));
...
Editor.prototype._refreshFakeCaret = function () {
    ...
    // We need to save the selection because adding the mark will mess
    // it up.  Rangy is of no use here because the method it uses to
    // save the region makes our node and offset coordinates unusable.
    var sel = this.getSelection();
    var ranges = [];
    ...
    for(var r_ix = 0, srange; (srange = ranges[r_ix]) !== undefined;
        ++r_ix) {
        sel.removeAllRanges();
...
Editor.prototype._inputHandler = log.wrap(function (e) {
    ...
    var caret = this._getRawCaret();
    var $caret = $(caret[0]);
    var $real = $caret.closest("._real");
    // We ingore such events outside of real data.
    if ($real.length) {
        if (caret[0].nodeType === Node.TEXT_NODE) {
            var ph = $caret.closest("._placeholder")[0];
            var data_caret, text_undo; // damn hoisting
            if (ph) {
                var parent = ph.parentNode;
                var index = _indexOf.call(ph.parentNode.childNodes, ph);
                data_caret = this.toDataCaret(parent, index);
                var text = $(ph).text().trim();
                if (text.length > 0) {
                    text_undo = this._initiateTextUndo();
                    var insert_ret =
                            this.data_updater.insertText(data_caret[0],
                                                         data_caret[1],
                                                         text);
                    var modified_node = insert_ret[0];
                    if (modified_node === undefined)
                        ...
                        var final_offset;
                        // Before the call, either the caret was in
                        // the text node that received the new text ...
                        if (modified_node === caret[0])
                            final_offset = caret[1] + text.length;
                        // ... or it was immediately adjacent to this text node.
                        else if (caret[0].childNodes[caret[1]] === modified_node)
                ...
                data_caret = this.getDataCaret();
                if (data_caret[0].nodeValue !== caret[0].nodeValue) {
                    // As usual, what ought to be simple is complex
                    // due to the way browsers work (Chrome 29 in this
                    // case). The upshot here is that if this input
                    // event happens by a composition which is ended
                    // by keyboard input, then the caret would be just
                    // *before* the text that the user
                    // entered. However, if the composition is ended
                    // by a mouse click then the caret will be *after*
                    // the composed text.

                    // Update the data caret before initiating the
                    // undo so that an undo won't cause a crash.
                    if (data_caret[1] > data_caret[0].nodeValue.length) {
                        data_caret[1] = data_caret[0].nodeValue.length;
                        this.setDataCaret(data_caret[0], data_caret[1]);
                    }

                    text_undo = this._initiateTextUndo();
                    this.data_updater.setTextNode(data_caret[0],
                                                  caret[0].nodeValue);
                    ...
                    this.setDataCaret(data_caret[0],
                                      data_caret[1], true);
                    text_undo.recordCaretAfter();
                }
            }
            ...
            throw new Error("input happened with caret outside text node");
    }
});
