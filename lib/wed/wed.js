...
<div class="row">\
 ...
 <div id="sidebar" class="col-sm-pull-10 col-lg-2 col-md-2 col-sm-2"/>\
    ...
    // separate.
    this.data_root = this.gui_root.cloneNode(true);
    ...
    this._gui_updater.addEventListener("deleteNode", function (ev) {
        var node = ev.node;
...
navigation panel brings up a contextual menu.</li>\
    ...
    this.merge_with_next_homogeneous_sibling_tr =
        new transformation.Transformation(
            this, "merge-with-next", "Merge <element_name> with next",
            function (editor, data) {
...
Editor.prototype._postInitialize = log.wrap(function  () {
    ...
    this.domlistener.addHandler(
        ...
        if (namespace === null && name.indexOf("data-wed", 0) === 0)
            this.validator.restartAt(el);
    }.bind(this));
...
Editor.prototype._fireTransformation = function(tr, transformation_data) {
    ...
    if (node !== undefined) {
        ...
        else if (!this.data_root.contains(node))
...
Editor.prototype._getMenuItemsForElement = function (node, offset, wrap) {
    ...
    function pushItem(data, tr) {
        ...
        menu_items.push({action: tr, item: $("<li></li>").append($a)[0],
                         data: data});
    }
    ...
    if (!node.classList.contains("_phantom") &&
        ...
        if (doc_url) {
            ...
            menu_items.push({action: null, item: $("<li></li>").append(a)[0],
                             data: null});
        }
...
Editor.prototype.computeContextMenuPosition = function (e) {
    ...
    // The next conditions happen only if the user is using the keyboard
        ...
        pos = {top: rect.top + this._$fake_caret.height() / 2,
               left: rect.left};
    }
    else {
...
function cut(editor, data) {
    ...
    for(var i = 0, limit = nodes.length; i < limit; ++i)
        editor._cut_buffer.appendChild(nodes[i]);
...
function paste(editor, data) {
    var $data = data.$data;
    var data_clone = $data[0].cloneNode(true);
    ...
    while(true)
    {
        ...
        if (closest_gui) {
                ...
                if (closest(node, "._element_name, ._attribute", root)) {
                    ...
                    if (next_attr) {
                        ...
                        var val = domutil.childByClass(next_attr,
                                                       "_attribute_value");
                        pos = pos.make(val.firstChild || val, 0);
                        break;
                    }
                }
            ...
            if (gui_before === closest_gui) {
                ...
                pos = pos.make(closest_gui, closest_gui.children.length);
                // ... and then out of it.
                continue;
            }
            // Normalize to the element name, and ...
            pos = pos.make(node.getElementsByClassName("_element_name")[0], 0);
            // ... stop here.
            break;
        }
...
Editor.prototype.positionLeft = function (pos) {
    ...
    // We are looking for the ``el`` such that ``pos.node`` is
    // FOLLOWING or CONTAINS it.
    function prevAttrFilter(el) {
        return (el.compareDocumentPosition(pos.node) &
    ...
    while(true)
    {
        var was_in_name = closestByClass(pos.node, "_element_name", root);
        ...
        if (closest_gui) {
            ...
            if (this.attributes === "edit" && start_label && !was_in_name) {
                ...
                // Search for the previous attribute.
                var prev_attr = _.find(
            ...
            if (!was_in_name) {
                pos = pos.make(
                    closest_gui.getElementsByClassName("_element_name")[0], 0);
                break;
            }
 ...
 * @private
 */
Editor.prototype._setGUICaret = function (loc, offset, text_edit) {
    var node;
    if (loc instanceof DLoc) {
        offset = loc.offset;
        node = loc.node;
        text_edit = arguments[1];
    }
    else {
        node = loc;
...
Editor.prototype._refreshFakeCaret = function () {
    ...
    // relative to the start of $gui_root.
    //
    var gr_position = this.gui_root.getBoundingClientRect();
    ...
    if (e.which >= 33 /* page up */ && e.which <= 40 /* down arrow */) {
        ...
        if (sel_focus_node) {
            ...
            if (gui && gui.classList.contains("_label_clicked")) {
                $(gui).trigger("wed-context-menu", [e]);
                return terminate();
            }
        }
...
Editor.prototype._isAttrValueProtected = function (attr_val) {
...
Editor.prototype._spliceAttribute = function (attr_val, offset, count, add) {
    ...
    if (this._isAttrValueProtected(attr_val))
        return;

    // If it has an element child, then this child is a transient
    // placeholdler, and this happens only when the value is empty.
    var val = (attr_val.firstElementChild || !attr_val.firstChild) ? "" :
            attr_val.firstChild.data;
    ...
    var name = domutil.siblingByClass(attr_val, "_attribute_name").textContent;
    this.data_updater.setAttribute(data_real, util.encodeAttrName(name), val);
    ...
    this._setGUICaret(move_to, offset, true);
    text_undo.recordCaretAfter();
};
...
Editor.prototype.elementAtPointUnderLayers = function (x, y) {
    var old_display = this._caret_layer.style.display;
    //
    ...
    this._caret_layer.style.display = "none";
    var element = this.my_window.document.elementFromPoint(x, y);
    this._caret_layer.style.display = old_display;
...
Editor.prototype._mousedownHandler = log.wrap(function(ev) {
    ...
    case 1:
        ...
        // GUI element, then normalize it to the start of the element.
            ...
            if (closestByClass(boundary.node, "_attribute_value", label))
                ...
                this.setGUICaret(
                    label.getElementsByClassName("_element_name")[0]
                        .firstChild, 0);
            }
        }
        else
    ...
    case 3:
        ...
        if (placeholder)
            this.setGUICaret(placeholder.firstChild, 0);

        if (label) {
            // Set the caret to be in the trigger.
            // Find the element name and put it there.
            if (closestByClass(target, "_attribute_value", label))
                this.setGUICaret(boundary);
            else {
                this.setGUICaret(
                    label.getElementsByClassName("_element_name")[0]
...
Editor.prototype.setDataCaret = function (loc, offset, text_edit) {
    ...
    this._setGUICaret(caret, text_edit);
};
...
Editor.prototype._caretChange = log.wrap(function (text_edit) {
    ...
    for(var i = 0, el; (el = this._caret_owners[i]) !== undefined; ++i)
        el.classList.remove("_owns_caret");

    if (old_caret) {
        var old_gui = closestByClass(old_caret.node, "_gui", old_caret.root);
        if (old_gui)
