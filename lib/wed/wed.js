...
(function () {
    ...
    this._postInitialize = function  () {
        ...
        this.copy_domlistener.addHandler(
            ...
            function ($root, $added, $removed, $prev, $next, $target) {
                ...
                var i, $node, $node_prev, $node_next;
                ...
                for(i = $removed_narrow.length - 1; i >= 0; --i) {
                    ...
                    if (mirror !== undefined) {
                        ...
                        if ($target.get(0).childNodes.length === 0) {
                            ...
                            $node_prev = (i === 0) ? $prev :
                                $removed_narrow.get( i- 1);
                            if ($node_prev.length > 0) {
                                path = domutil.nodeToPath(
                                    $root.get(0), $node_prev.get(0));
                                $(this.pathToNode(path)).next().remove();
                            }
                            else {
                                // It is the first text node.
                                $mirror = $($target.data("wed_mirror_node"));
                                mirror = $mirror.get(0);
                                for(var m_ix = 0;
                                    m_ix < mirror.childNodes.length; ++m_ix) {
                                    if (mirror.childNodes[m_ix].nodeType === Node.TEXT_NODE) {
                                        $(mirror.childNodes[m_ix]).remove();
                                        break;
                                    }
                                }
                            }
    ...
    this._fireTransformation = function (e) {
        ...
        this._undo.startGroup(
            new wundo.UndoGroup("Undo " +
                               ...
                               this));
        var node;
        if (e.data.node !== undefined) {
            var path = this.nodeToPath(e.data.node);
            node = domutil.pathToNode(this.$tree_root.get(0), path);
        }
        tr.handler(this, node, e.data.element_name);
        this.domlistener.processImmediately();
        this._undo.endGroup();
    ...
    this.toTreeCaret = function(node, offset) {
        ...
        if ($(child).is("._placeholder, ._gui, ._wed_caret")) {
            ...
            var $prev = $(node.childNodes[offset]).prev(":not(._placeholder, ._gui, ._wed_caret)");
            if ($prev.length === 0) {
                tree_node = domutil.pathToNode(this.$tree_root.get(0),
                                               this.nodeToPath(node));
                return [tree_node, 0];
