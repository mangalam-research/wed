...
Editor.prototype.init = log.wrap(function (widget, options) {
    ...
    this.$gui_root.wrap('<div class="row"><div id="wed-frame" class="col-lg-10"><div class="row"/></div></div>');
...
Editor.prototype._postInitialize = log.wrap(function  () {
    ...
    this.gui_domlistener.addHandler(
        ...
        if ($added.is("._real, ._phantom_wrap") ||
            ...
            var index = _indexOf.call(parent.childNodes, target);
            this._gui_updater.removeNode(target);
            this._gui_updater.insertAt(parent, index, $children.toArray());
        }
    });
    ...
    // the placeholder
    var updating_placeholder = 0;
    this.gui_domlistener.addHandler(
        "children-changed",
        "._real, ._phantom_wrap",
        function ($root, $added, $removed, $prev, $next, $target) {
        if (updating_placeholder)
            return;
        ...
        try {
            ...
            if ($to_consider.length === 0 ||
                ($to_consider.legnth === 1 && $to_consider.is($removed))) {
                ...
                this._gui_updater.removeNodes(
                    $target.children("._placeholder").toArray());
        }
        finally {
            updating_placeholder--;
        }
    ...
    this.$gui_root.on('keydown',
                      jqutil.eventHandler(this._keydownHandler.bind(this)));
    this.$gui_root.on('wed-global-keydown',
                      this._globalKeydownHandler.bind(this));

    this.$gui_root.on('keypress',
                      jqutil.eventHandler(this._keypressHandler.bind(this)));
    this.$gui_root.on('wed-global-keypress',
                      this._globalKeypressHandler.bind(this));
    this.$gui_root.on('click.wed',
                      jqutil.eventHandler(this._mouseHandler.bind(this)));
    ...
    this.$gui_root.on('keyup mouseup',
                      jqutil.eventHandler(this._caretChangeEmitter.bind(this)));
    this.$gui_root.on('mousedown',
                      jqutil.eventHandler(this._mousedownHandler.bind(this)));
    ...
    this.$gui_root.on('paste',
                      jqutil.eventHandler(this._pasteHandler.bind(this)));

    this.$gui_root.on('cut',
                      jqutil.eventHandler(this._cutHandler.bind(this)));
    $(this.my_window).on('resize.wed', this._resizeHandler.bind(this));

    this.$gui_root.on('click', 'a', function () {
...
Editor.prototype._postInitialize2 = log.wrap(function (ns_mapping) {
    ...
    this.$gui_root.find(".head").each(function (x, el) {
        nav_links.push("<li><a class='navbar-link' href='#" + el.id + "'>" + $(el).text() +
                       "</a></li>");
    });
...
Editor.prototype._contextMenuHandler = function (e, jQthis) {
    ...
    if (range) {
        ...
        originally_collapsed = range.collapsed;
...
Editor.prototype._globalKeydownHandler = log.wrap(function (wed_event, e,
    ...
    if (this._development_mode) {
        ...
        if (e.which === 113) {
            ...
            console.log("data closest real",
                        $(this.getDataCaret()[0]).closest("._real"));
    ...
    else if (key_constants.CTRL_PERIOD.matchesEvent(e)) {
...
Editor.prototype._handleKeyInsertingText = function (e) {
    ...
    this._syncDisplay();
    if (insert_ret[0] === undefined)
        this.setDataCaret(insert_ret[1], text.length);
    else
        this.setDataCaret(insert_ret[0], caret[1] + text.length);
    e.preventDefault();
    e.stopPropagation();
};

Editor.prototype._mouseHandler = function (e, jQthis) {
    if (e.type === "click") {
        this._undo.endAllGroups();
...
Editor.prototype._mousedownHandler = function(e, jQthis) {
    this.$widget.find('.wed-validation-error').removeClass('selected');
    var range = this.getSelectionRange();
    if (!range || range.collapsed) {
        this.pushSelection();
        this.my_window.setTimeout(this._seekCaret.bind(this, e, jQthis,
                                                       e.clientX,
                                                       e.clientY), 0);
    }
};

Editor.prototype._seekCaret = function (ev, jQthis, x, y) {
    ...
    if (ev.which === 3) {
        if (in_gui)
            $target.trigger("wed-context-menu", [ev, jQthis]);
        else
...
Editor.prototype.getDataCaretAsPath = function () {
    var caret = this.getDataCaret();
    return [this.data_updater.nodeToPath(caret[0]), caret[1]];
};

Editor.prototype.setDataCaretAsPath = function (caret) {
    var real_caret = [this.data_updater.pathToNode(caret[0]),
                      caret[1]];
...
Editor.prototype._caretChangeEmitter = function (ev, jQthis) {
    if (ev === undefined)
    ...
    if (ev.type === "mouseup" || ev.type === "click")
        window.setTimeout(this._caretChangeEmitterTimeout.bind(this, ev), 0);
    else
        this._caretChangeEmitterTimeout(ev);
};

Editor.prototype._caretChangeEmitterTimeout = log.wrap(function (ev) {
    if (ev.type === "mouseup" || ev.type === "click") {
        ...
        if ($(ev.target).closest("._placeholder").length > 0) {
            r = rangy.createRange(this.my_window.document);
            r.setStart(ev.target, 0);
            this.getSelection().setSingleRange(r);
        }
    ...
    if (this._raw_caret === undefined ||
        ...
        this.$gui_root.trigger("caretchange", [this._raw_caret, old_caret]);
    }
});

Editor.prototype._caretChangeHandler = log.wrap(function (e, caret, old_caret) {
...
Editor.prototype.displayContextMenu = function (x, y, items) {
    ...
    $menu.on('keydown mouseleave mouseenter',
             jqutil.eventHandler(this._menuHandler.bind(this)));
...
Editor.prototype._normalizeSelectionRange = function () {
    ...
    var me = this;
    function _normalize(container, offset) {
        if(container.nodeType === Node.ELEMENT_NODE) {
            // Normalize to a range within the editable nodes. We could be
            // outside of them in an element which is empty, for instance.
            var pair = me.mode.nodesAroundEditableContents(container);
    ...
    var start = _normalize(range.startContainer, range.startOffset);
    var end = _normalize(range.endContainer, range.endOffset);
...
Editor.prototype._processValidationError = function (ev) {
    ...
    $marker.click(jqutil.eventHandler(function (ev, jQthis) {
    ...
    $item.children("a").click(jqutil.eventHandler(function (ev, jQthis) {
        this.$widget.find('.wed-validation-error').removeClass('selected');
...
Editor.prototype._rangeToPixelPosition = function() {
    ...
    var offset;
    if (range) {
        range = this.getSelectionRange().cloneRange();
        var start_is_focus = ((selection.focusNode === range.startContainer) &&
                              (selection.focusOffset === range.startOffset));
        range.collapse(start_is_focus);
