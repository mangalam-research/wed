...
<div class="modal hide fade" tabindex="1">\
  ...
  <div class="modal-footer">\
    ...
    this.validation_domlistener = new domlistener.Listener(this.root);
...
(function () {
    ...
    this._postInitialize = function  () {
        ...
        this.domlistener.addHandler(
            ...
            function ($root, $added, $removed, $prev, $next, $target) {
                ...
                $added = $added.filter("._real, ._phantom_wrap, ._placeholder");
                $removed = $removed.filter("._real, ._phantom_wrap, ._placeholder");
                ...
                this._undo.record(new wundo.DOMUndo(
                    this, $target, $added, $removed, $prev, $next));
            }.bind(this));
    ...
    this._fireTransformation = function (e) {
        ...
        this._undo.startGroup(
            new wundo.UndoGroup("Undo " +
                                tr.getDescriptionFor(e.data.element_name),
                               this));
    ...
    this._keyDownHandler = function (e, jQthis) {
        ...
        if (e.which === 46 &&
            e.charCode === 0 &&
            $(domutil.nextCaretPosition(this._raw_caret, this.root, true)[0])
            .is("._phantom, ._phantom_wrap"))
            return false;

        if (e.which === 8 &&
            e.charCode === 0  &&
            $(domutil.prevCaretPosition(this._raw_caret, this.root, true)[0])
            .is("._phantom, ._phantom_wrap"))
            return false;
    };
    ...
    this._keyPressHandler = function (e) {
        ...
        var current_group = this._undo.getGroup();
        if (current_group === undefined)
            this._undo.startGroup(new wundo.TextUndoGroup(this._undo, 10,
                                                          "text"));
        ...
        (function () {
            var modified_node_path;
            var text_node_path;
            var original_offset;
            var text = String.fromCharCode(e.which);
            var undo = new wundo.Undo("Undo insert " + text,
                                      this,
                                      e.target,
                                      function (editor, node) {
                                          var caret = editor.getCaret();
                                          var insert_ret =
                                                  domutil.insertText(caret[0],
                                                                     caret[1],
                                                                     text);
                                          modified_node_path =
                                              editor.nodeToPath(insert_ret[0]);
                                          text_node_path =
                                              editor.nodeToPath(insert_ret[1]);
                                          original_offset = caret[1];
                                      ...
                                      function (editor, node) {
                                          ...
                                          if (modified === text) {
                                              domutil.deleteText(
                                                  ...
                                                  original_offset,
                                                  1);
                                              editor.setCaret(text,
                                                              original_offset);
                                          }
                                          else {
                                              text.parentNode.
                                                  removeChild(text);
                                              editor.setCaret(modified,
                                                              original_offset,
                                                              true);
                                          }
                                      });
