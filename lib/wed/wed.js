...
function Editor(widget, options) {
    ...
    var $document = $("<div class='wed-document span10'/>");
    $document.css("overflow-y", "auto");
    $document.css("max-height", window.innerHeight - 
                  $(this.widget).position().top -
                  ($(document).height() - $(this.widget).next().position().top));

    $(this.widget).wrapInner($document);
    // jQuery does not update $document to reflect its position in the
    // DOM tree.
    $document = $(this.widget.childNodes[0]);
    
    this.root = $document.get(0);

    $document.first().wrap('<div class="row-fluid"/>');
    this.$sidebar = $('<div id="sidebar" class="span2"/>');
    $(this.widget).find('.row-fluid').first().prepend(this.$sidebar);
    // Needed by Parser
    this.$sidebar.prepend("<div id='tmp-state'>&nbsp;</div>");
    this.$sidebar.append('\
<div class="accordion">\
...
(function () {
        ...
        if (!$node_of_interest.hasClass("_phantom")) {
            ...
            this.parser.possibleAt(
                ...
                range.startOffset).forEach(function (ev) {
                    if (ev.params[0] === "endTag" || ev.params[0] === "text")
                        return;
    ...
    var state_to_str = {};
    state_to_str[parser.STOPPED] = "stopped";
    ...
    this._onParserStateChange = function () {
        ...
        if (working_state.state === parser.WORKING) {
            ...
            message += " " + ((working_state.part_done * 100) >> 0) + "%";
            this._last_done_shown = working_state.part_done;
        }
        
        $(this.widget).find("#tmp-state").text(message);
    };
    ...
    this._onParserError = function (ev) {
        ...
        var $item = $("<li><a href='#" + element.id + "'>" + error + "</li>");
