...
Editor.prototype.isAttrProtected = function (attr, parent) {
    ...
    if (attr instanceof this.my_window.Attr) {
        name = util.decodeAttrName(attr.name);
    }
...
function boundaryX(boundary) {
    ...
    if (((node_type === Node.ELEMENT_NODE) && (node.childNodes.length === 0)) ||
        ...
        var parent = node.parentNode;
        return boundaryX(parent, _indexOf.call(parent.childNodes, node));
    }
        ...
        if (((node_type === Node.ELEMENT_NODE) &&
            ...
            case Node.TEXT_NODE:
                ...
                if (right && !left)
                    ...
                    boundary = Math.abs(boundaryX(right) - x) >=
                        Math.abs(boundaryX(left) - x) ? left : right;
                break;
            case Node.ELEMENT_NODE:
    ...
    function moveToNormalizedLabelPosition() {
        ...
        if (attr) {
            if (closestByClass(boundary.node, "_attribute_value", label))
                ...
                var val = attr.getElementsByClassName("_attribute_value")[0];
                me.setGUICaret(val.firstChild ? val.firstChild : val, 0);
            }
        }
        else {
