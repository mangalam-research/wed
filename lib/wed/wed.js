...
Editor.prototype._contextMenuHandler = function (e) {
    ...
    this.displayContextMenu(action_context_menu.ContextMenu, pos.left, pos.top,
                            menu_items);
    return false;
};
...
Editor.prototype.positionRight = function (pos) {
    ...
    var closest_gui = closestByClass(pos.node, "_gui", root);
    if (closest_gui)
    ...
    while(true)
    {
        pos = pos.make(domutil.nextCaretPosition(pos.toArray(),
        ...
        closest_gui = closest(node, "._gui:not(._invisible)", root);
        if (closest_gui) {
...
Editor.prototype._refreshFakeCaret = function () {
    ...
    if (!this._sel_focus ||
        ...
        closestByClass(node, "_gui", this.gui_root)) {
        if (this._fake_caret.parentNode)
            this._fake_caret.parentNode.removeChild(this._fake_caret);
...
Editor.prototype._globalKeydownHandler = log.wrap(function (wed_event, e) {
    ...
    if (e.which >= 33 /* page up */ && e.which <= 40 /* down arrow */) {
        ...
        if (sel_focus_node &&
            sel_focus_node.classList &&
            sel_focus_node.classList.contains("_label_clicked")) {
...
Editor.prototype._findLocationInElementAt = function(node, x, y, text_ok) {
    ...
    // Utility function
    function checkRangeNormal(node, start) {
        var rects;
        if (node.nodeType === Node.TEXT_NODE) {
    ...
    function checkRangeIE(node, start) {
        if (node.nodeType === Node.TEXT_NODE &&
            start > 0 && start < node.length - 1) {
            range.setStart(node, start - 1);
            range.setEnd(node, start);
            var prev_rect = range.getBoundingClientRect();
            range.setStart(node, start + 1);
            range.setEnd(node, start + 2);
            var next_rect = range.getBoundingClientRect();

            // The characters before and after the one we are
            // interested in must be on different lines.
            ...
            if (rect.top === prev_rect.top &&
                rect.bottom === next_rect.bottom)
                return false;
        }
...
Editor.prototype._mousedownHandler = log.wrap(function(ev) {
    ...
    var gui = closestByClass(target, "_gui", root);
    switch(ev.which) {
    case 1:
...
Editor.prototype._normalizeCaret = function (loc) {
    ...
    var pg = closest(loc.node, "._placeholder, ._gui", loc.root);
...
Editor.prototype._caretChange = log.wrap(function (text_edit) {
    ...
    if (this._sel_focus) {
        ...
        if (focus_node.nodeType === Node.ELEMENT_NODE) {
            ...
            var ph = domutil.childByClass(focus_node, "_placeholder");
