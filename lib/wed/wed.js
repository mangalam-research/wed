...
Editor.prototype.init = log.wrap(function (widget, options, data) {
    ...
    if (data) {
        ...
        this._data_doc = parser.parseFromString(data, "application/xml");
    }
    else {
        this._data_doc = null;
    }
...
Editor.prototype._postInitialize = log.wrap(function  () {
    ...
    if (this._data_doc && this._data_doc.firstChild)
...
function cut(editor, data) {
    ...
    if (start_caret.node instanceof editor.my_window.Attr) {
        ...
        for(var i = 0, limit = nodes.length; i < limit; ++i)
            doc.firstElementChild.appendChild(nodes[i]);
        editor._cut_buffer.textContent = doc.firstElementChild.innerHTML;
...
Editor.prototype._pasteHandler = function(e) {
    ...
    var as_xml = true;
    if (doc.firstElementChild.tagName === "parsererror" &&
        doc.firstElementChild.namespace ===
        "http://www.mozilla.org/newlayout/xml/parsererror.xml") {
        as_xml = false;
    }

    if (as_xml) {
        data = doc.firstElementChild;
