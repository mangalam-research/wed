...
Editor.prototype._refreshFakeCaret = function () {
    ...
    //
    // getSelectionRange ensures we don't get a range if the current
...
Editor.prototype._mousemoveHandler = log.wrap(function (e) {
    ...
    //var range = rangy.createRange(this.my_window.document);
    var range = this.my_window.document.createRange();
    if (rangy.dom.comparePoints(
        this._sel_anchor.node, this._sel_anchor.offset,
        this._sel_focus.node, this._sel_focus.offset) < -1) {
        range.setStart(this._sel_anchor.node, this._sel_anchor.offset);
        range.setEnd(this._sel_focus.node, this._sel_focus.offset);
        this.getSelection().setSingleRange(range);
    }
    else {
        range.setStart(this._sel_focus.node, this._sel_focus.offset);
        range.setEnd(this._sel_anchor.node, this._sel_anchor.offset);
        this.getSelection().removeAllRanges();
        this.getSelection().addRange(range, true);
    }
...
Editor.prototype._caretChangeEmitterTimeout = log.wrap(
    ...
    if (ev.type === "mouseup" && this._sel_focus)
