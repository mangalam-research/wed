        ...
        if ($data_node_of_interest.get(0) !== this.data_root.childNodes[0]) {
            ...
            if (trs !== undefined) {
                trs.forEach(function (tr) {
                    var data = $.extend({}, data_base,
                                        {node: $data_node_of_interest.get(0),
                                         element_name: orig });
                    var icon = tr.getIcon();
                    var $a = $("<a tabindex='0' href='#'>" +
                                (icon ? icon + " " : "") +
...
function cut(editor, data) {
    ...
    var start_caret = editor.toDataCaret(range.startContainer,
                                         range.startOffset);
    var end_caret = editor.toDataCaret(range.endContainer, range.endOffset);
...
Editor.prototype._globalKeydownHandler = log.wrap(function (wed_event, e) {
    ...
    if (key_constants.BACKSPACE.matchesEvent(e)) {
        ...
        if (!$(domutil.prevCaretPosition(raw_caret, this.gui_root, true)[0])
            ...
            // delete. Handle this.
            if (caret[0].nodeType !== Node.TEXT_NODE)
                caret = [caret[0].childNodes[caret[1] - 1],
                         caret[0].childNodes[caret[1] - 1].length];
...
Editor.prototype.elementAtPointUnderLayers = function (x, y) {
    ...
    var element;
    try {
        element = this.my_window.document.elementFromPoint(x, y);
    }
    finally {
...
Editor.prototype._seekCaret = log.wrap(function (ev) {
    var range = this._normalizeSelectionRange();
