    ...
    this.moveCaretRight = function () {
        ...
        while(true)
        {
            ...
            if (// Can't stop inside a phantom node.
                (($(pos[0]).closest("._phantom").length > 0) ||
                 // Or beyond the first position in a placeholder
                 // node.
                 (pos[1] > 0 &&
                  $(pos[0]).closest("._placeholder").length > 0)))
                continue;
            ...
            if (pos[0].nodeType === Node.ELEMENT_NODE) {
                ...
                var prev_node = (pos[1] > 0) ?
                    pos[0].childNodes[pos[1] - 1]:
                    undefined;

                if (next_node === undefined ||
                    ($(next_node).is("._gui") &&
                     // gui node at start of an element
                     (prev_node === undefined ||
                      // between gui nodes
                      $(prev_node).is("._gui") ||
                      // gui node at the end of an element;
                      (pos[0].childNodes.length -1 === pos[1]))))
    ...
    this._keyDownHandler = function (e, jQthis) {
        ...
        if ($placeholders.length > 0) {
            ...
            if (!util.anySpecialKeyHeld(e) &&
                !this.parser.possibleAt(caret[0], caret[1]).
