...
Editor.prototype._postInitialize = function  () {
    ...
    this.copy_domlistener.addHandler(
        ...
        // Don't worry about DOM nodes that are no longer part of the tree.
        if ($target.closest($root).length === 0)
            return;
        var editor = this;
    ...
    this.copy_domlistener.addHandler(
        ...
        for(i = 0; i < $added_narrow.length; ++i) {
            ...
            if ($node_prev.length > 0) {
                path = domutil.nodeToPath($root.get(0), $node_prev.get(0));
        ...
        for(i = $removed_narrow.length - 1; i >= 0; --i) {
            ...
            if (mirror !== undefined) {
                ...
                if ($target.get(0).childNodes.length === 0) {
                    ...
                    // We can't do the following because the node is
                    // no longer in its tree.
                    //
                    // path = domutil.nodeToPath($root.get(0),
                    //                           $node.get(0));
                    // $(this.pathToNode(path)).remove();

                    // This is relatively expensive but there is no
                    // easy way currently to locate text nodes that
            ...
            // else: it can happen that a node has no mirror data and
            // is not a text node. Consider the following:
            //
            // - <p><a/><b/></p> is added
            // - <b/> is removed from <p>
            //
            // Then the events are processed by the copy_listener.  By
            // the time the addition of <p> is acted on, <p> is as
            // follows: <p><a/></p> because <b/> has been removed, so
            // <b/> is never linked. Then the removal of <b/> is
            // processed but it has no mirror because it was never
            // linked.
        }
...
Editor.prototype.fireTransformation = function(tr, node, element_name,
    ...
    if (current_group !== undefined) {
        if (!(current_group instanceof wundo.TextUndoGroup))
            throw new Error("unexpected group class in _fireTransformation");
        this._undo.endGroup();
    }
