...
Editor.prototype._findBoundaryAt = function (x, y) {
    var old_display = this._$fake_caret.css("display");
    this._$fake_caret.css("display", "none");
    var element_at_mouse =
            this.my_window.document.elementFromPoint(x, y);
    // This could happen if x, y is outside our screen.
    if (!element_at_mouse)
        return undefined;
    this._$fake_caret.css("display", old_display);
...
Editor.prototype.destroy = function () {
    ...
    try {
        ...
        $('body', this.my_window.document).off('.wed');
...
// to be a reliably portable way to get the position of a range
// otherwise.
//
...
Editor.prototype._rangeToPixelPosition = function() {
    ...
    case Node.TEXT_NODE:
        ...
        ret = $marker.offset();
        // Adjust to the center of the line.
        ret.top += $marker.height() / 2;
...
// LocalWords:  contenteditable Ok Ctrl onbeforeunload css minified
// LocalWords:  SimpleEventEmitter stylesheet hoc tabindex revalidate
// LocalWords:  compositionupdate compositionend mousemove mouseup ns
// LocalWords:  mousedown xml namespace xmlns enterStartTag jshint
// LocalWords:  validthis prepend offscreen getDOMSelectionRange prev
// LocalWords:  insertIntoText CapsLock uneditable startContainer Yay
// LocalWords:  endContainer getCaret TextUndoGroup keydown keypress
// LocalWords:  keyup compositionstart RTL toDataCaret caretchange
// LocalWords:  setCaret caretChangeEmitter focusNode rethrown
// LocalWords:  saveSelection
