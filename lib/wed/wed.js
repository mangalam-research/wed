...
Editor.prototype._postInitialize = log.wrap(function  () {
    ...
    this.domlistener.addHandler(
        ...
        if (element.classList.contains("_placeholder"))
            element.classList.add("_dying");

        if (!this._sel_focus)
            return; // no caret to move

        var caret = this._sel_focus.node;
        if (element.contains(caret)) {
            // We must move the caret to a sane position.
            if (prev && root.contains(parent) && root.contains(prev))
                this.setGUICaret(parent, _indexOf.call(parent.childNodes,
                                                       prev) + 1);
            else if (next && root.contains(parent) && root.contains(next))
                this.setGUICaret(parent,
                                 _indexOf.call(parent.childNodes, next));
            else if (root.contains(parent))
