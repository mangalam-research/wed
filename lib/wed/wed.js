...
Editor.prototype.init = function (widget, options) {
    ...
    this.$gui_root.wrap('<div class="row-fluid"><div class="span10"/></div>');
    this.$sidebar = $('<div id="sidebar" class="span2"/>');
    this.$widget.find('.row-fluid').first().prepend(this.$sidebar);
    // Needed by Validator
    this.$gui_root.before("<div class='row-fluid'><div class='span12 progress progress-info'><span></span><div id='validation-progress' class='bar' style='width: 0%'></div></div></div>");
...
(function () {
    ...
    this._postInitialize = function  () {
        ...
        this.copy_domlistener = new domlistener.Listener(this.$data_root);
        ...
        this.copy_domlistener.addHandler(
            ...
            function ($root, $added, $removed, $prev, $next, $target) {
                ...
                for(i = $removed_narrow.length - 1; i >= 0; --i) {
                    ...
                    // else: it can happen that a node has no mirror
                    // data and is not a text node. Consider the following:
                    //
                    // - <p><a/><b/></p> is added
                    // - <b/> is removed from <p>
                    //
                    // Then the events are processed by the
                    // copy_listener.  By the time the addition of <p>
                    // is acted on, <p> is as follows: <p><a/></p>
                    // because <b/> has been removed, so <b/> is never
                    // linked. Then the removal of <b/> is processed
                    // but it has no mirror because it was never
                    // linked.
                }

                if ($removed_narrow.length > 0)
                    // The caret might have become stale.
        ...
        // added-element because the copy_domlistener may add entire
        // trees.
        this.domlistener.addHandler(
            "included-element",
            "._real, ._phantom_wrap",
            function ($root, $tree, $parent, $prev, $next, $target) {
                if ($target.children(
                    '._real, ._phantom._text, ._phantom_wrap').length
                    === 0 &&
                    $target.contents().filter(jqutil.textFilter).length
                    === 0) {
                    var nodes =
                            this.mode.nodesAroundEditableContents(
                                $target.get(0));
                    var ph = this.mode.makePlaceholderFor($target.get(0));
                    if (nodes[0] !== null)
                        $(nodes[0]).after(ph);
        ...
        if (this.gui_root.childNodes.length === 0) {
            ...
            var evs = this.validator.possibleAt(this.gui_root, 0).toArray();
            if (evs.length === 1 && evs[0].params[0] === "enterStartTag") {
                transformation.insertElement(
                    this, this.$data_root.get(0), 0,
        ...
        this.$gui_root.find(".head").each(function (x, el) {
            nav_links.push("<li><a href='#" + el.id + "'>" +
                           $(el).text() + "</a></li>");
        });
    ...
    /**
     * @param {module:transformation~Transformation} tr The
     * transformation to fire.
     * @param {Node} node The DOM node to fire it on. This can be a
     * node from the GUI tree or from the data tree.
     ...
     * transformation.
     * @param {Array} new_caret_position A new caret position in the
     * data tree.
     */
    ...
    this._contextMenuHandler = function (e, jQthis) {
        ...
        if (!$node_of_interest.hasClass("_phantom") &&
            ...
            $node_of_interest = $(domutil.pathToNode(this.$data_root.get(0),
    ...
    this._globalKeydownHandler = function (wed_event, e, jQthis) {
        function terminate() {
            e.stopPropagation();
            e.preventDefault();
            return false;
    ...
    this._keypressHandler = function (e, jQthis) {
    ...
    this._globalKeypressHandler = function (wed_event, e, jQthis) {
        if (this._raw_caret === undefined)
            return true;
    ...
    this.fromDataCaret = function (node, offset) {
        // Accept a single array as argument
        if (arguments.length === 1 && node instanceof Array) {
            offset = node[1];
            node = node[0];
        ...
        var gui_child = this.pathToNode(
            domutil.nodeToPath(this.$data_root.get(0),
                               node.childNodes[offset]));
    ...
    this.toDataCaret = function(node, offset) {
        // Accept a single array as argument
        if (arguments.length === 1 && node instanceof Array) {
            offset = node[1];
            node = node[0];
    ...
    this.getDataCaret = function () {
        this.copy_domlistener.processImmediately();
        var caret = this.getCaret();
        return this.toDataCaret(caret);
    };

    this.setDataCaret = function (node, offset) {
        this.copy_domlistener.processImmediately();
        // Accept a single array as argument
        if (arguments.length === 1 && node instanceof Array) {
            offset = node[1];
    ...
    this.getDataCaretAsPath = function () {
        var caret = this.getDataCaret();
        return [domutil.nodeToPath(this.$data_root.get(0), caret[0]),
                caret[1]];
    };

    this.setDataCaretAsPath = function (caret) {
        var real_caret = [domutil.pathToNode(this.$data_root.get(0), caret[0]),
                          caret[1]];
        this.setDataCaret(real_caret);
    };

    this.toDataNode = function (node) {
        return domutil.pathToNode(this.$data_root.get(0),
                                  this.nodeToPath(node));
    };
    ...
    this.destroy = function () {
        ...
        try {
            if (this.copy_domlistener !== undefined)
                this.copy_domlistener.stopListening();
        } catch(ex) { recorded = ex; }
