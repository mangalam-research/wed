...
<div id="sidebar-panel" class="panel-group wed-sidebar-panel">\
 <div class="panel wed-navigation-panel">\
...
Editor.prototype._findLocationInElementAt = function(node, x, y, text_ok) {
    ...
    function checkRange(node, start) {
        ...
        if (!min || min.dist.y > dist.y ||
            ...
            // Return true to indicate that the search can end.
...
Editor.prototype._pointToCharBoundary = function(x, y) {
    ...
    if (boundary) {
        ...
        case Node.ELEMENT_NODE:
            ...
            var before;
            var pointed_node = boundary.node.childNodes[boundary.offset];
            if (pointed_node.nodeType ===  Node.ELEMENT_NODE) {
                var closest = this._findLocationInElementAt(pointed_node, x, y);
                var limit = (closest.node.nodeType === Node.ELEMENT_NODE) ?
                        closest.node.childNodes.length - 1 :
                        -1;
            ...
            if (!before)
                boundary = boundary.make(boundary.node, boundary.offset + 1);

            break;
        default:
            throw new Error("unexpected node type: " + boundary.node.nodeType);
        }
    }
    return boundary;
};
