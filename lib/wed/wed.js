    ...
    if (key_constants.DELETE.matchesEvent(e)) {
        ...
        if (!$(domutil.nextCaretPosition(raw_caret, this.gui_root, true)[0])
            ...
            if (caret[0].nodeType === Node.TEXT_NODE) {
                ...
                this._undo.getGroup().recordCaretAfter();
            }
        }
        return terminate();
    }
...
Editor.prototype._handleKeyInsertingText = function (e) {
    ...
    if (insert_ret[0] === undefined)
        ...
        this.setDataCaret(insert_ret[0], caret[1] + text.length, true);
    this._undo.getGroup().recordCaretAfter();
    e.preventDefault();
    e.stopPropagation();
};
...
/**
 * Converts a gui caret to a data caret.
 *
 * @param {Node|Array} node The node where the caret is.
 * @param {Integer} [offset] The offset in that node. If this
 * parameter is not set, then the first parmeter must be an array of
 * two elements, containing the node and the offset in this order.
...
Editor.prototype.toDataCaret = function(node, offset) {
    ...
    if ($(node).closest("._phantom, ._gui").length > 0)
        return undefined;
