 ...
 */
define(/** @lends module:key */ function f(require, exports) {
  "use strict";
   ...
   * otherwise.
   */
  Key.prototype.matchesEvent = function matchesEvent(ev) {
   ...
   * the method.
   */
  Key.prototype.setEventToMatch = function setEventToMatch(ev) {
    ev.which = this.which;
    ev.keyCode = this.keyCode;
    ev.charCode = this.charCode;
    ev.ctrlKey = this.ctrlKey;
   ...
   */
  Key.prototype.hash = function hash() {
    return this._id;
  };
   ...
   * purposes.
   */
  Key.prototype.anyModifier = function anyModifier() {
  ...
  /**
   * This function creates a key object which represents a meta
   * character (a character typed while Meta is held).
   *
   *
   * @param {string|number} ch This parameter can be a string of
   * length one which contains the character for which we want to create
  ...
  /**
   * This function creates a key object which represents a "control
   * equivalent" character. A "control equivalent" is equivalent to a
   * control key on all platforms, except in OS X where it is equivalent
   * to a command key. That is if one makes a "control equivalent" with
   * the character "X", then on all platforms it would be equivalent to
   * hitting Ctrl-X, except in OS X where it is equivalent to hitting
   * Command-X.
   *
   * @param {string|number} ch This parameter can be a string of
   * length one which contains the character for which we want to create
