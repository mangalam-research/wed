 ...
 */
define(/** @lends module:lib/conditioned */ function f(require, exports) {
  "use strict";

  /**
   * This constructor must be called by the class using this mixin.
   *
   * @classdesc <p>This class is a mixin which adds the glue needed for
   * an object to allow other objects to know when certain conditions
   * have been met. This mixin depends on an event emitting and
   * listening framework of the sort provided by {@link
   * module:lib/simple_event_emitter simple_event_emitter}. The object
   * using this mixin does not have to use {@link
   * module:lib/simple_event_emitter simple_event_emitter} but must
   * offer the same interface.</p>
   *
   * <p>Since this mixin builds on {@link
   * module:lib/simple_event_emitter simple_event_emitter} or something
   * similar, the following are true:
   *
   *   <ul>
   *
   ...
   * being executed, but the condition won't be recorded as being in
   * effect as would be the case if {@link
   * module:lib/conditioned~Conditioned#_setCondition _setCondition} would
   * be called.</p>
   * @mixin
   *
   * @constructor
   ...
   * condition.
   */
  Conditioned.prototype._setCondition = function _setCondition(name, ev) {
   ...
   * call {@link module:lib/conditioned~Conditioned#whenCondition
   ...
   * already, or a falsy value if the condition has not been met.
   */
  Conditioned.prototype.getCondition = function getCondition(name) {
   ...
   */
  Conditioned.prototype.whenCondition = function whenCondition(name, listener) {
...
//  LocalWords:  falsy ev mixin namespace Mangalam MPL Dubeau
