...
Transformation.prototype.getDescriptionFor = function (element_name) {
...
function insertElement(editor, parent, index, tag_name, attrs, contents) {
    ...
    case Node.TEXT_NODE:
        domutil.insertIntoText(parent, index, $new.get(0));
        break;
...
function unwrap(node) {
    ...
    if (node.childNodes.length === 0)
        ...
        var $prev = $node.prev();
        if ($prev.length === 0)
            $parent.prepend(node.childNodes);
        else
...
var split_node_tr = new Transformation("Split <element_name>",
                                       splitNode);

var merge_with_previous_homogeneous_sibling_tr =
        new Transformation("Merge <element_name> with previous",
                           mergeWithPreviousHomogeneousSibling);

var merge_with_next_homogeneous_sibling_tr =
        new Transformation("Merge <element_name> with next",
...
exports.mergeWithNextHomogeneousSibling =
        mergeWithNextHomogeneousSibling;
exports.split_node_tr = split_node_tr;
exports.merge_with_next_homogeneous_sibling_tr =
        merge_with_next_homogeneous_sibling_tr;
exports.merge_with_previous_homogeneous_sibling_tr =
