...
function makeSplitMergeInputTrigger(editor, element_name, split_key,
    ...
    ret.addKeyHandler(
        ...
        if (type === "keypress" ||
            ...
            editor.fireTransformation(split_node_on, $el.get(0), undefined,
...
var split_node_on = new transformation.Transformation(
    "Split node on character",
    ...
    while(modified) {
        ...
        for (var i = 0; !modified && i < text_nodes.length; ++i) {
            ...
            if (node.firstChild === text && offset === 0) {
                ...
                text.nodeValue = text.nodeValue.slice(1);
                modified = true;
            }
                ...
                text.nodeValue =
                    text.nodeValue.slice(0, text.nodeValue.length - 1);
                modified = true;
            }
            else if (offset !== -1) {
                var pair = domutil.splitAt(node, text, offset);
