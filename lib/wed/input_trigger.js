 ...
 * @param {string} selector This is a CSS selector. The object created by this
 * constructor will listen to events that pertain only to DOM nodes
...
InputTrigger.prototype._keydownHandler = function (wed_event, e) {
    ...
    var node_of_interest = closest(caret.node, this._selector, caret.root);

    if (node_of_interest) {
        node_of_interest = this._editor.toDataNode(node_of_interest);
...
InputTrigger.prototype._pasteHandler = function (wed_event, e, caret, data) {
    ...
    if (!closest(node_of_interest, this._selector, caret.root))
        return;
