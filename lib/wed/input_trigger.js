...
function InputTrigger(editor, selector) {
    ...
    editor.data_domlistener.addHandler("children-changed",
                                       selector,
                                       this._childrenChanged.bind(this));

    editor.data_domlistener.addHandler("text-changed",
                                       selector,
 ...
 * <code>"keypress"</code> for keypress events,
 ...
 * <code>event</code> is the jQuery event in question. It is present
 * only when the type of event is <code>"keydown"</code> or
 ...
 * the handler will be called once, even though two ";" are added. It
 * is up to the handler to detect that ";" has been added more than
 * once.</p>
 *
...
//  LocalWords:  keydown tabindex keypress
