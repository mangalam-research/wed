...
define(/** @lends module:wed */function f(require) {
  ...
  EditorP._contextMenuHandler = function _contextMenuHandler(e) {
    ...
    if (!sel || (!sel.collapsed && !sel.wellFormed)) {
      return false;
    }
    ...
    var menu_items = method.call(this, node, offset, !sel.collapsed);
    ...
    function _moveToNormalizedLabelPosition(target, label, boundary) {
      ...
      if (attr) {
        if (closestByClass(boundary.node, "_attribute_value", label)) {
          this.caretManager.setCaret(boundary);
        }
        else {
          this.caretManager.setCaret(
            getAttrValueNode(
        ...
        this.caretManager.setCaret(
          label.getElementsByClassName("_element_name")[0], 0);
      }
    };
  ...
  EditorP._mousedownHandler = log.wrap(function _mousedownHandler(ev) {
    ...
    case 1:
      ...
      if (placeholder) {
        this.caretManager.setCaret(placeholder, 0);
      }
      else if (label) {
        // If the caret is changing due to a click on a
  ...
  EditorP._mouseupHandler = log.wrap(function _mouseupHandler(ev) {
    ...
    case 3:
      ...
      if (placeholder) {
        this.caretManager.setCaret(target, 0);
      }
  ...
  EditorP._setupCompletionMenu = function _setupCompletionMenu() {
    ...
    if (attr_val) {
      ...
      var data_caret = this.caretManager.getDataCaret();
  ...
  EditorP.increaseLabelVisibilityLevel =
    function increaseLabelVisibilityLevel() {
      ...
      var pos = this.caretManager.caretPositionOnScreen();
      ...
      var pos_after = this.caretManager.caretPositionOnScreen();
