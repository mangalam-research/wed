...
define(/** @lends module:wed */function f(require) {
  ...
  EditorP._contextMenuHandler = function _contextMenuHandler(e) {
    ...
    var node = this.caretManager.caret.node;
    var offset = this.caretManager.caret.offset;
  ...
  EditorP._getMenuItemsForElement = function _getMenuItemsForElement(node,
    ...
    if (!node.classList.contains("_phantom") &&
      ...
      var tree_caret = this.caretManager.toDataLocation(node, offset);
  ...
  EditorP.computeContextMenuPosition = function computeContextMenuPosition(
    ...
    // The next conditions happen only if the user is using the keyboard
      ...
      var gui = closestByClass(this.caretManager.caret.node, "_gui", this.gui_root);
      if (gui) {
    ...
    if (e.which >= 33 /* page up */ && e.which <= 40 /* down arrow */) {
      ...
      if (key_constants.RIGHT_ARROW.matchesEvent(e)) {
        if (e.shiftKey) {
          ...
          pos = this.caretManager.caretPositionRight();
          rr = this.caretManager.anchor.makeRange(pos);
          this.setSelectionRange(rr.range, rr.reversed);
        }
        else {
          this.caretManager.moveRight();
        }
        return terminate();
      }
        ...
        if (e.shiftKey) {
          // Extend the selection
          pos = this.caretManager.caretPositionLeft();
          rr = this.caretManager.anchor.makeRange(pos);
          this.setSelectionRange(rr.range, rr.reversed);
        }
        else {
          this.caretManager.moveLeft();
        }
        return terminate();
      }
        ...
        if (e.shiftKey) {
          // Extend the selection
          pos = this.caretManager.caretPositionDown();
          rr = this.caretManager.anchor.makeRange(pos);
          this.setSelectionRange(rr.range, rr.reversed);
        }
        else {
          this.caretManager.moveDown();
        }
        return terminate();
      }
        ...
        if (e.shiftKey) {
          // Extend the selection
          pos = this.caretManager.caretPositionUp();
          rr = this.caretManager.anchor.makeRange(pos);
          this.setSelectionRange(rr.range, rr.reversed);
        }
        else {
          this.caretManager.moveUp();
        }
        return terminate();
      }
      return true;
        ...
        if (!next.classList ||
          ...
          if (caret.node.nodeType === Node.TEXT_NODE) {
            ...
            if (caret.node.parentNode) {
              this.setCaret(caret, { textEdit: true });
            }
            else {
              this.setCaret(caret.make(parent, offset), { textEdit: true });
            }
      ...
      if (attr_val) { // In attribute.
        ...
        if (!prev.classList ||
          ...
          if (caret.node.nodeType === Node.TEXT_NODE) {
            ...
            // deleted.
            if (caret.node.parentNode) {
              this.setCaret(caret.makeWithOffset(caret.offset - 1),
                            { textEdit: true });
            }
            else {
              this.setCaret(caret.make(parent, offset), { textEdit: true });
  ...
  EditorP._cutSelection = function _cutSelection() {
    ...
    if (sel && !sel.collapsed) {
      ...
      this.setCaret(cut_ret[0], { textEdit: true });
      text_undo.recordCaretAfter();
      return true;
    }
    ...
    function _moveToNormalizedLabelPosition(target, label, boundary) {
      ...
      if (attr) {
        if (closestByClass(boundary.node, "_attribute_value", label)) {
          this.setCaret(boundary);
        }
        else {
          this.setCaret(getAttrValueNode(attr.getElementsByClassName(
            "_attribute_value")[0]), 0);
        }
      }
      else {
        // Find the element name and put it there.
        this.setCaret(label.getElementsByClassName("_element_name")[0], 0);
      }
    };
  ...
  EditorP._mousedownHandler = log.wrap(function _mousedownHandler(ev) {
    ...
    case 1:
      ...
      // _sel_focus and caretManager.anchor were set by setCaret.
