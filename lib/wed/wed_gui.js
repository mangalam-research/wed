...
define(/** @lends module:wed */function f(require) {
  ...
  var action_context_menu = require("./gui/action-context-menu");
  var completion_menu = require("./gui/completion-menu");
  ...
  EditorP._getMenuItemsForElement = function _getMenuItemsForElement(node,
    ...
    if (!node.classList.contains("_phantom") &&
      ...
      trs = this.getElementTransformationsAt(tree_caret,
                                             wrap ? "wrap" : "insert");
      ...
      if (data_node !== this.data_root.firstChild) {
        trs = this.mode.getContextualActions(
          ["unwrap", "delete-parent", "split"], data_node.tagName, data_node, 0);
        for (tr_ix = 0; tr_ix < trs.length; ++tr_ix) {
  ...
  EditorP.computeContextMenuPosition = function computeContextMenuPosition(
    ...
    else if (this._caretMark.inDOM) {
      var rel_pos = this._caretMark.getPositionFromGUIRoot();
      rect = this._caretMark.getBoundingClientRect();
      this.scrollIntoView(rel_pos.left, rel_pos.top,
                          rel_pos.left + rect.width,
                          rel_pos.top + rect.height);
      // We need to refresh immediately and reacquire the client rectangle of
      // the caret.
      this._caretMark.refresh();
      rect = this._caretMark.getBoundingClientRect();
      pos = {
  ...
  EditorP.scrollIntoView = function scrollIntoView(left, top, right, bottom) {
    // Adjust gui_root.
    var vtop = this._scroller.scrollTop;
    var vheight = this._scroller.offsetHeight;
    var vbottom = vtop + vheight;

    if (top < vtop || bottom > vbottom) {
      // Not already in view.
      vtop = top < vtop ? top : bottom - vheight;
      this._scroller.scrollTop = vtop;
    }

    var vleft = this._scroller.scrollLeft;
    var vwidth = this._scroller.offsetWidth;
    var vright = vleft + vwidth;

    if (left < vleft || right > vright) {
      // Not already in view.
      vleft = left < vleft ? left : right - vwidth;
      this._scroller.scrollLeft = vleft;
    }

    var pos = this._scroller.getBoundingClientRect();
  ...
  EditorP._globalKeydownHandler = log.wrap(function _globalKeydownHandler(
    ...
    if (e.which >= 33 /* page up */ && e.which <= 40 /* down arrow */) {
      ...
      if (key_constants.RIGHT_ARROW.matchesEvent(e)) {
        ...
        if (e.shiftKey) {
          // Extend the selection
          this._sel_focus = this.positionDown(this._sel_focus);
          rr = this._sel_anchor.makeRange(this._sel_focus);
          this.setSelectionRange(rr.range, rr.reversed);
        }
        else {
        ...
        if (e.shiftKey) {
          // Extend the selection
          this._sel_focus = this.positionUp(this._sel_focus);
          rr = this._sel_anchor.makeRange(this._sel_focus);
          this.setSelectionRange(rr.range, rr.reversed);
        }
        else {
  ...
  EditorP._setupCompletionMenu = function _setupCompletionMenu() {
    ...
    if (attr_val) {
      ...
      // First see if the mode has somethign to say.
      var possible = this.mode.getAttributeCompletions(data_caret.node);
      ...
      this._current_dropdown =
        new completion_menu.CompletionMenu(
          ...
          function done() {
            var dropdown = this._current_dropdown;
  ...
  EditorP._onValidatorStateChange = function _onValidatorStateChange(
    ...
    var part_done = working_state.partDone;
  ...
  EditorP._refreshValidationErrors = function _refreshValidationErrors() {
    this._refreshValidationErrorsRunner.reset();
    this._resumeTaskWhenPossible(this._refreshValidationErrorsRunner);
  };
   ...
   * @private
   */
  EditorP._clearValidationErrors = function _clearValidationErrors() {
    this._validation_errors = [];
    this._processed_validation_errors_up_to = -1;
    this._refreshValidationErrorsRunner.stop();
    this._processValidationErrorsRunner.stop();

    var list = this._$error_layer[0];
    while (list.lastChild != null) {
      list.removeChild(list.lastChild);
    }

    list = this.$error_list[0];
    while (list.lastChild != null) {
      list.removeChild(list.lastChild);
    }

    this._refreshValidationErrorsRunner.reset();
    this._processValidationErrorsRunner.reset();
  };

  /**
   * This method recreates the error messages and the error markers
   * associated with the errors that the editor already knows.
   *
   * @private
   */
  EditorP._recreateValidationErrors = function _recreateValidationErrors() {
    var list = this._$error_layer[0];
    while (list.lastChild != null) {
      list.removeChild(list.lastChild);
    }
    ...
    for (var ix = 0; ix < this._validation_errors.length; ++ix) {
      var error = this._validation_errors[ix];
  ...
  function getGUINodeIfExists(editor, node) {
    try {
      var caret = editor.fromDataLocation(node, 0);
      return caret && caret.node;
    }
    catch (ex) {
  ...
  EditorP._processValidationErrors = function _processValidationErrors() {
    ...
    this._processValidationErrorsRunner.reset();
    this._resumeTaskWhenPossible(this._processValidationErrorsRunner);
  };

  EditorP._processValidationError = function _processValidationError(err) {
    var ev = err.ev;
    var error = ev.error;
    var data_node = ev.node;
    var index = ev.index;

    var isAttributeNameError = error instanceof salve.AttributeNameError;

    var insert_at;

    if (isAttributeNameError) {
      var guiNode = getGUINodeIfExists(this, data_node);
      if (!guiNode) {
        return false;
      }
      var insertionNode = guiNode.querySelector(
        "._gui.__start_label ._greater_than");
      insert_at = dloc.DLoc.mustMakeDLoc(this.gui_root, insertionNode, 0);
    }
    else {
      insert_at = findInsertionPoint(this, data_node, index);
    ...
    if (!marker) {
      var doc = insert_at.node.ownerDocument;
      ...
      if (!invisible_attribute) {
        ...
        link.href = "#" + marker_id;
        link.textContent = error.toStringWithNames(converted_names);
        item.appendChild(link);

        $(item.firstElementChild).click(this._errorItemHandler_bound);
      }
      else {
        item.textContent = error.toStringWithNames(converted_names);
  ...
  EditorP.increaseLabelVisibilityLevel =
    function increaseLabelVisibilityLevel() {
      ...
      this._recreateValidationErrors();
      this._caretMark.refresh();
