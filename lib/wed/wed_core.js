...
define(/** @lends module:wed */function f(require, exports) {
    ...
    try {
      ...
      if (!attr_val) {
        ...
        if (modified_node === undefined) {
          this.setCaret(caret.make(insert_ret[1], text.length),
                        { textEdit: true });
        }
        else {
          var final_offset;
          ...
          this.setCaret(caret.make(modified_node, final_offset),
                        { textEdit: true });
        }
  ...
  EditorP._spliceAttribute = function _spliceAttribute(attr_val, offset,
    ...
    this._setGUICaret(this.caretManager.makeCaret(move_to, offset), { textEdit: true });
    text_undo.recordCaretAfter();
  };
