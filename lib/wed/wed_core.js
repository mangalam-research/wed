...
define(/** @lends module:wed */function f(require, exports) {
  ...
  function Editor() {
    ...
    this.firstValidationComplete = new Promise(function make(resolve) {
      this.firstValidationCompleteResolve = resolve;
    }.bind(this));

    this.initialized = new Promise(function make(resolve) {
   ...
   */
  EditorP.fireTransformation = function fireTransformation(tr,
                                                           transformation_data) {
   ...
   * ``_exitTaskSuspension`` should be called the same number of times to resume
   * the tasks.
   */
  EditorP._enterTaskSuspension = function _enterTaskSuspension() {
    if (this._taskSuspension === 0) {
      this._stopAllTasks();
    }
    this._taskSuspension++;
  };
   ...
   * ``_enterTaskSuspension`` was called.
   */
  EditorP._exitTaskSuspension = function _exitTaskSuspension() {
    this._taskSuspension--;
    if (this._taskSuspension < 0) {
      throw new Error("_exitTaskSuspension underflow");
    }
    if (this._taskSuspension === 0) {
      this._resumeAllTasks();
    }
  };
   ...
   */
  EditorP._stopAllTasks = function _stopAllTasks() {
    for (var runnerIx = 0; runnerIx < this._taskRunners.length; ++runnerIx) {
      var runner = this._taskRunners[runnerIx];
      runner.stop();
    }
   ...
   */
  EditorP._resumeAllTasks = function _resumeAllTasks() {
    for (var runnerIx = 0; runnerIx < this._taskRunners.length; ++runnerIx) {
      var runner = this._taskRunners[runnerIx];
      runner.resume();
    }

    // The validator is a special case. And yes, ``start`` is the correct
   ...
   * ``_enterTaskSuspension``, resume the task right away. Otherwise, this is a
   * no-op.
   */
  EditorP._resumeTaskWhenPossible = function _resumeTaskWhenPossible(task) {
    if (this._taskSuspension === 0) {
      task.resume();
    }
  };

  EditorP._fireTransformation = function _fireTransformation(
    tr, transformation_data) {
    ...
    var caret = transformation_data.moveCaretTo;
    if (caret) {
  ...
  EditorP.recordUndo = function recordUndo(undo) {
    this._undo.record(undo);
  };

  EditorP.undo = function undo() {
    // We need to replicate to some extent how fireTransformation
    // inhibits functions and reinstates them.
    this.caretManager.mark.suspend();
    this._enterTaskSuspension();
  ...
  EditorP.dumpUndo = function dumpUndo() {
    console.log(this._undo.toString()); // eslint-disable-line no-console
  };

  EditorP.undoMarker = function undoMarker(msg) {
    this.recordUndo(new wundo.MarkerUndo(msg));
  };

  EditorP.undoingOrRedoing = function undoingOrRedoing() {
   ...
   */
  EditorP.isAttrProtected = function isAttrProtected(attr, parent) {
    var name;
   ...
   */
  EditorP.save = function save() {
    return this.saver.save();
  };

  EditorP._initiateTextUndo = function _initiateTextUndo() {
  ...
  EditorP._terminateTextUndo = function _terminateTextUndo() {
    var current_group = this._undo.getGroup();
    if (current_group instanceof wundo.TextUndoGroup) {
      this._undo.endGroup();
    }
  ...
  EditorP._normalizeEnteredText = function _normalizeEnteredText(text) {
  ...
  EditorP._compensateForAdjacentSpaces = function _compensateForAdjacentSpaces(
    text, caret) {
    if (!this._normalize_entered_spaces) {
      return text;
    }
  ...
  EditorP._insertText = function _insertText(text) {
    ...
    try {
      ...
      if (!attr_val) {
        ...
        if (modified_node === undefined) {
          this.caretManager.setCaret(insert_ret[1], text.length,
                                     { textEdit: true });
        }
        else {
          var final_offset;
          ...
          this.caretManager.setCaret(modified_node, final_offset,
                                     { textEdit: true });
        }
  ...
  EditorP._spliceAttribute = function _spliceAttribute(attr_val, offset,
                                                       count, add) {
    if (offset < 0) {
      return;
    }
    ...
    var resolved = this.mode.getAbsoluteResolver().resolveName(name, true);
    ...
    this.caretManager.setCaret(move_to, offset, { textEdit: true });
    text_undo.recordCaretAfter();
  };
   ...
   */
  EditorP.insertTransientPlaceholderAt = function insertTransientPlaceholderAt(
    loc) {
  ...
  EditorP.toDataNode = function toDataNode(node) {
  ...
  EditorP.fromDataNode = function fromDataNode(node) {
    if (node.nodeType === Node.ELEMENT_NODE) {
      var ret = $.data(node, "wed_mirror_node");
      // We can bypass the whole pathToNode, nodeToPath thing.
      if (ret) {
    ...
    return this.pathToNode(this.data_updater.nodeToPath(node));
  };

  EditorP._onSaverSaved = function _onSaverSaved() {
    notify("Saved", { type: "success" });
    this._refreshSaveStatus();
  };

  EditorP._onSaverAutosaved = function _onSaverAutosaved() {
    notify("Autosaved", { type: "success" });
    this._refreshSaveStatus();
  };

  EditorP._onSaverChanged = function _onSaverChanged() {
    this._refreshSaveStatus();
  };

  EditorP._onSaverFailed = function _onSaverFailed(event) {
    this._refreshSaveStatus();
    var me = this;
    ...
    if (error.type === "too_old") {
      ...
      this._disconnect_modal.modal(function done() {
        me.save();
      });
    }
    else if (error.type === "save_edited") {
      this._edited_by_other_modal.modal(function done() {
  ...
  EditorP.nodeToPath = function nodeToPath(node) {
    return this.gui_dloc_root.nodeToPath(node);
  };

  EditorP.pathToNode = function pathToNode(path) {
    return this.gui_dloc_root.pathToNode(path);
  };

  EditorP.getModeData = function getModeData(key) {
    return this._mode_data[key];
  };

  EditorP.setModeData = function setModeData(key, value) {
    this._mode_data[key] = value;
  };

  EditorP.destroy = function destroy() {
