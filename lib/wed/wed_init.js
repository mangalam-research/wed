...
define(/** @lends module:wed */function f(require) {
  ...
  var EditingMenuManager = require("./gui/editing-menu-manager").EditingMenuManager;
  ...
  var ModeTree = require("./mode-tree").ModeTree;
  ...
  EditorP.init = log.wrap(function init(widget, options, xmlData) {
    this._max_label_level = undefined;
...
<div class=\"row\">\
 ...
 <div id=\"sidebar\" class=\"col-sm-pull-10 col-lg-2 col-md-2 col-sm-2\"/>\
    ...
    this.validation_message = this.validation_progress.previousElementSibling;
...
");
    ...
    return this.modeTree.init().then(function modeLoaded(tree) {
      return this.onModeChange(tree.getMode(this.gui_root));
    }.bind(this));
  });
  ...
  EditorP.onModeChange = log.wrap(function onModeChange(mode) {
    return Promise.resolve()
      .then(function load() {
        // We purposely do not raise an error here so that calls to destroy
        // can be done as early as possible. It aborts the initialization
        // sequence without causing an error.
        if (this._destroyed) {
          return undefined;
        }
        this.mode = mode;

        this.max_label_level = this.modeTree.getMaxLabelLevel();
        this._current_label_level = this.modeTree.getInitialLabelLevel();

        var styles = this.modeTree.getStylesheets();
        for (var style_ix = 0; style_ix < styles.length; ++style_ix) {
          this.$frame.children("head").append(
            "<link rel=\"stylesheet\" href=\"" + styles[style_ix] +
              "\" type=\"text/css\" />");
        }
        ...
        var modeTree = this.modeTree;
        function moveInAttributes(node) {
      ...
      .then(function loaded(schema) {
        this.validator = new Validator(schema, this.data_root,
...
wed's generic help. The link by default will open in a new tab.</p>");
        ...
        if (save) {
          ...
          savePromise = this.runtime.resolveModules(save.path)
            .then(function loaded(modules) {
              var Saver = modules[0].Saver;
              var saver = new Saver(this.runtime, version, this.data_updater,
                                    this.data_root, save.options);
              this.saver = saver;
              ...
              if (save.autosave !== undefined) {
                saver.setAutosaveInterval(save.autosave * 1000);
              }
              ...
              onbeforeunload.install(this.my_window,
                                     function beforeunload() {
                                       ...
                                       return !!this.saver.getModifiedWhen();
                                     }.bind(this),
                                     true);
        ...
        return savePromise.then(function initialized() {
  ...
  EditorP._initializeNamespaces = function _initializeNamespaces() {
    var resolver = this.mode.getAbsoluteResolver();
    var failure = false;
      ...
      if (evs.length === 1 && evs[0].params[0] === "enterStartTag") {
        ...
        if (name.simple() && name.toArray().length === 1) {
          transformation.insertElement(
            this.data_updater, this.data_root, 0, name.ns,
            resolver.unresolveName(name.ns, name.name), attrs);
