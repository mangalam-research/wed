...
define(/** @lends module:wed */function f(require) {
  ...
  var object_check = require("./object-check");
  ...
  var RefreshValidationErrors = require("./tasks/refresh-validation-errors").RefreshValidationErrors;
  var ProcessValidationErrors = require("./tasks/process-validation-errors").ProcessValidationErrors;
  var CaretMark = require("./caret-mark").CaretMark;
  ...
  EditorP.init = log.wrap(function init(widget, options, xmlData) {
    ...
    if (options instanceof Runtime) {
      ...
      // If the user passed options straight, then make a runtime from
      // them.
 ...
 <div class=\"panel panel-danger\">\
  ...
  <div id=\"sb-errors-collapse\" data-parent=\"#sidebar-panel\"\
   ...
   <div id=\"sb-errors\" class=\"panel-body\">\
    ...
    this._caretMark = new CaretMark(this, doc,
                                    this._caret_layer,
                                    this._$input_field[0],
                                    this._scroller);
...
");
    ...
    this._refreshValidationErrorsRunner =
      this._newTaskRunner(new RefreshValidationErrors(this));
    this._processValidationErrorsRunner =
      this._newTaskRunner(new ProcessValidationErrors(this));
  ...
  EditorP.setMode = log.wrap(function setMode(mode_path, options) {
    var onload = log.wrap(function onload(mode_module) {
      this._new_mode = new mode_module.Mode(this, options);
      this._new_mode.init().then(function modeReady() {
        this.onModeChange(this._new_mode);
      }.bind(this));
    }).bind(this);

    require([mode_path], onload, function onError() {
      ...
      require([path], onload, function onFinalError() {
  ...
  EditorP._postInitialize = log.wrap(function _postInitialize(error) {
    ...
    this._$scroller.on("scroll", this._caretMark.boundRefresh);
    ...
    this.$gui_root.on("focus", log.wrap(function focus(ev) {
    ...
    this._$error_layer.on("mousedown click contextmenu", function mousedown(ev) {
