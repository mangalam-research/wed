...
<div class=\"row\">\
 ...
 <div id=\"sidebar\" class=\"col-sm-pull-10 col-lg-2 col-md-2 col-sm-2\"/>\
    ...
    this._gui_updater.addEventListener(
      "beforeDeleteNode", function beforeDelete(ev) {
        var node = ev.node;
...
");
    ...
    pubsub.subscribe(pubsub.WED_MODE_READY, function ready(msg, mode) {
  ...
  EditorP.onModeChange = log.wrap(function onModeChange(mode) {
    ...
    this.validator = new Validator(this.runtime,
                                   this.options.schema,
                                   this.data_root,
                                   mode_validator);
  ...
  EditorP._postInitialize = log.wrap(function _postInitialize() {
    if (this._destroyed) {
      return;
    }
