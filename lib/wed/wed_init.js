...
");
    ...
    this.modeTree.init().then(function modeLoaded(tree) {
      this.onModeChange(tree.getMode(this.gui_root));
    }.bind(this));
  });
...
wed's generic help. The link by default will open in a new tab.</p>");
    ...
    if (save) {
      ...
      this.runtime.resolveModules(save.path)
        .then(function loaded(modules) {
          ...
          return saver.init();
        }.bind(this)).then(function initialized() {
