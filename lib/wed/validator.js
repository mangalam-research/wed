...
define(/** @lends module:validator */ function f(require, exports) {
   ...
   *
   * @constructor
   * @param {module:runtime~Runtime} runtime The runtime in which this validator
   * is used.
   ...
   */
  function Validator(runtime, schema, root, mode) {
   ...
   */
  ValidatorP.start = function start() {
    ...
    if (!this._initialized) {
      this.initialize(function initialized() {
   ...
   */
  ValidatorP.initialize = function initialize(done) {
    if (this.schema instanceof salve.Grammar) {
      ...
      this.runtime.resolve(this.schema).then(function then(x) {
   ...
   */
  ValidatorP.getSchemaNamespaces = function getSchemaNamespaces() {
    if (!this._initialized) {
   ...
   */
  ValidatorP.getDocumentNamespaces = function getDocumentNamespaces() {
    var ret = {};
   ...
   * @private
   */
  ValidatorP._workWrapper = function _workWrapper() {
    if (this._work()) {
      this._timeout_id = window.setTimeout(this._bound_wrapper,
                                           this._timeout);
    }
   ...
   * otherwise.
   */
  ValidatorP._work = function _work() {
    if (!this._initialized) {
      return true;
    }
   ...
   */
  ValidatorP._cycle = function _cycle() {
   ...
   */
  ValidatorP.restartAt = function restartAt(node) {
    if (this._working_state === WORKING) {
      this.stop();
    }
  ...
  ValidatorP._resetTo = function _resetTo(node) {
   ...
   */
  ValidatorP._setWorkingState = function _setWorkingState(new_state, new_done) {
    var changed = false;
   ...
   */
  ValidatorP.getWorkingState = function getWorkingState() {
    return {
   ...
   * @emits module:validator~Validator#error
   */
  ValidatorP._processEventResult = function _processEventResult(result, node,
                                                                index) {
    for (var ix = 0; ix < result.length; ++ix) {
      var error_data = { error: result[ix], node: node, index: index };
   ...
   * @emits module:validator~Validator#error
   */
  ValidatorP._processError = function _processError(error) {
   ...
   * @private
   */
  ValidatorP._fireAttributeEvents = function _fireAttributeEvents(walker, el) {
   ...
   * @private
   */
  ValidatorP._fireAttributeNameEvent = function _fireAttributeNameEvent(walker,
                                                                        el,
                                                                        attr) {
   ...
   * in ``el``.
   */
  ValidatorP._fireAndProcessEvent = function _fireAndProcessEvent(walker,
                                                                  event,
                                                                  el, ix) {
   ...
   */
  ValidatorP._validateUpTo = function _validateUpTo(container, index,
                                                    attributes) {
   ...
   */
  ValidatorP._getWalkerAt = function _getWalkerAt(container, index,
                                                  attributes) {
    attributes = !!attributes; // Normalize.
    if (attributes && (!container.childNodes ||
                       container.childNodes[index].nodeType !==
   ...
   */
  ValidatorP.possibleWhere = function possibleWhere(container, event) {
    var ret = [];
    for (var index = 0; index <= container.childNodes.length; ++index) {
      ...
      if (possible.has(event)) {
        ...
        for (var ix = 0; ix < as_array.length; ++ix) {
          var candidate = as_array[ix];
  ...
  ValidatorP.speculativelyValidateFragment =
    function speculativelyValidateFragment(container, index, to_parse) {
      ...
      var dup = new Validator(this.runtime, this._tree, to_parse);
   ...
   */
  ValidatorP.getErrorsFor = function getErrorsFor(node) {
  ...
  function isPossibleDueToWildcard(walker, event_name, ns, name) {
    ...
    var matched = false;
    for (var ev_ix = 0; ev_ix < evs.length; ++ev_ix) {
      var ev = evs[ev_ix];
   ...
   *
   */
  ValidatorP._setPossibleDueToWildcard = function _setPossibleDueToWildcard(
    node, walker, event_name, ns, name) {
