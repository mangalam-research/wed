...
Validator.prototype.getDocumentNamespaces = function () {
    ...
    var def = this.root.querySelector("[data-wed-xmlns]");
    if (def)
        ret[""] = [def.getAttribute("data-wed-xmlns")];
...
Validator.prototype._cycle = function () {
    ...
    while (true) {
        ...
        case START_TAG:
            ...
            // "_real _text" elements do not generate events.
            if (!cur_el.classList.contains("_text")) {
        ...
        case CONTENTS:
            ...
            while (node !== null) {
                ...
                case Node.TEXT_NODE:
                    var event = new validate.Event("text", node.data);
...
Validator.prototype._resetTo = function (node) {
    ...
    function erase(el) {
        ...
        for(var ix = 0, limit = el.children.length; ix < limit; ++ix)
            erase(el.children[ix]);
    }
    erase(this.root);
    this._validation_stage = CONTENTS;
    this._previous_child = null;
...
Validator.prototype._fireAttributeEvents = function (walker, el) {
    ...
    for(var i = 0, attr; (attr = el.attributes[i]); ++i) {
        ...
        this._fireAndProcessEvent(
            walker,
            new validate.Event("attributeValue", attr.value), el, 0);
    }
};
...
Validator.prototype._fireAttributeNameEvent = function (walker, el, attr) {
    ...
    this._fireAndProcessEvent(
        walker,
        new validate.Event("attributeName", ename.ns, ename.name), el, 0);
};
...
Validator.prototype._validateUpTo = function (container, index, attributes) {
    ...
    if (attributes && (container.nodeType !== Node.ELEMENT_NODE ||
    ...
    if (container === this.root && index <= 0) {
        ...
        case Node.ELEMENT_NODE:
            ...
            // to_inspect's default is fine for the next few options
                ...
                data_key = !attributes ? "wed_event_index_after_start" :
                "wed_event_index_after_attributes";
            break;
...
Validator.prototype._getWalkerAt = function(container, index, attributes) {
    ...
    if (container === this.root && index <= 0) {
        ...
        event_index = container.wed_event_index_after;
    ...
    function fireTextEvent(text_node) {
        var event = new validate.Event("text", text_node.data);
    ...
    case Node.ELEMENT_NODE:
        ...
        if (!attributes) {
            ...
            if (prev_sibling &&
                ...
                else if (prev_sibling.classList.contains("_text"))
                    fireTextEvent(prev_sibling.firstChild);
            }
        }
        break;
    case Node.ATTRIBUTE_NODE:
        ...
        event_index = el.wed_event_index_before_attributes;
