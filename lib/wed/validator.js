        ...
        while (true) {
            ...
            case START_TAG:
                ...
                if (!$element_to_validate.hasClass("_phantom_wrap") &&
                    ...
                    var attr_ix_lim =
                            this._element_to_validate.attributes.length;
                    for(var attr_ix = 0; attr_ix < attr_ix_lim; ++attr_ix) {
                        var attr =
                                this._element_to_validate.attributes[attr_ix];
                    ...
                    var name = this._element_to_validate.className.split(" ",
                                                                         1)[0];
                    var ename = this.resolver.resolveName(name);
                    this._fireAndProcessEvent(
                        walker,
                        new validate.Event("enterStartTag", ename.ns,
                                           ename.name),
                ...
                this._validation_stage = CONTENTS;
                $element_to_validate.data(
                    "wed_event_index_after_start", this._events.length - 1);
            ...
            case CONTENTS:
                ...
                while (node !== null) {
                    ...
                    case Node.TEXT_NODE:
                        ...
                        // text is possible.
                        event_result =
                            this._fireTextEventIfNeeded(node, walker);
                        if (event_result)
                            // XXX
            ...
            case END_TAG:
                ...
                if (!$element_to_validate.hasClass("_phantom_wrap") &&
                    ...
                    var name = this._element_to_validate.className.
                            split(" ", 1)[0];
                    var ename = this.resolver.resolveName(name);
                    this._fireAndProcessEvent(
                        walker,
                ...
                this._element_to_validate =
                    this._element_to_validate.parentNode;
...
Validator.prototype.restartAt = function (element) {
    if (this._working_state === WORKING)
        this.stop();
...
Validator.prototype.possibleAt = function (container, index) {
    ...
    case Node.TEXT_NODE:
        ...
        // location is inside the current text node or if another text
        // node appeared between this text node and the previous
        // element found.
        if (index > 0 ||
