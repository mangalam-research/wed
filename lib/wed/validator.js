...
function Validator(schema, root) {
    ...
    $(this._element_to_validate).data("wed_event_index_after_start",
                                      this._events.length);
 ...
 * caret is.</p>
 *
 * <p>There is one exception in the way the <code>container,
 * index</code> pair is interpreted. If the container is the
 ...
 * document. For instace, unclosed elements won't be reported.</p>
 *
 * @private
...
Validator.prototype.speculativelyValidate = function (container, index,
    ...
    // Calling _getWalkerAt has for side effect to bring the resolver up
    // to the state we want.
    dup.resolver = this.resolver.clone();
