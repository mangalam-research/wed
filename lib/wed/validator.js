 ...
 * contains it the document to validate but is not
...
function Validator(schema, root) {
    ...
    this.schema = schema;
    this.resolver = new name_resolver.NameResolver();
...
/**
 * Controler method for the background validation. Keeps the validator
...
/**
 * Peforms one cycle of validation. "One cycle" is an arbitrarily
 * small unit of work.
 *
 * @private
...
Validator.prototype._cycle = function () {
    ...
    while (true) {
        ...
        case START_TAG:
            ...
            // $element_to_validate.data("wed_state_index_before",
            // this._events.length - 1);
            // $element_to_validate.data("wed_event_index_before",
            // this._events.length - 1);
            ...
            if (!$element_to_validate.hasClass("_phantom_wrap") &&
                !$element_to_validate.is(
                    "._real._text, ._placeholder, ._wed_caret")) {
                ...
                for(attr_ix = 0; attr_ix < attr_ix_lim; ++attr_ix) {
                    ...
                    ename = this.resolver.resolveName(attr_name, true);
                    this._fireAndProcessEvent(
                        walker,
            ...
            this._validation_stage = CONTENTS;
            $element_to_validate.data("wed_event_index_after_start",
                                      this._events.length);
        ...
        case CONTENTS:
            ...
            while (node !== null) {
                ...
                case Node.TEXT_NODE:
                    ...
                    // is not just white space or if text is possible.
        ...
        case END_TAG:
            ...
            if (this._element_to_validate === this.root) {
                ...
                if (event_result)
                    this._processTagEventResult(event_result,
                                                this._element_to_validate);
                this._part_done = 1;
                this._setWorkingState(this._errors.length > 0 ? INVALID :
                                      VALID);
                $(this._element_to_validate).data("wed_event_index_after",
                                                  this._events.length);
                this.stop();
                return false;
            }
            ...
            // we need it later
            var original_element = this._element_to_validate;
 ...
 * stopped validator has for side effect to start it.)
 *
...
/**
 * Resets validation to continue from aspecific point. Any further
 ...
 * <code>part_done</code> is the part of validation done so far.
 */
 ...
 * @param {Node} element The element to which the result belongs.
 */
Validator.prototype._processTagEventResult = function (result, element) {
    for(var ix = 0, err; (err = result[ix]) !== undefined; ++ix) {
        this._errors.push(err);
        this._emit("error", { error: err, element: element});
    }
};
 ...
 * @param {Node} element_to_validate The DOM node associated with this event.
 */
Validator.prototype._fireAndProcessEvent = function (walker, event,
                                                     element_to_validate) {
...
/**
 * Force immediate validation up to the point specified by the
...
/**
 * Exception to be raised if we can't find our place in the events
 ...
 * point expressed by the parameters. These parameters are interpreted
 * in the same way a DOM caret is. This method forces validation up to
 * the point specified.
 *
 * @private
 *
 ...
 * the parameters. A "possible event" is an event that if fired won't
 * result in a validation error. These parameters are interpreted in
 * the same way a DOM caret is. This method forces validation up to
 * the point specified.
 *
 * @param {Node} container
 * @param {Integer} index
 ...
 * document. This parameters should be X.
 */
