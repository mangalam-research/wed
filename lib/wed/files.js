    ...
    files.config(function ($provide) {
        $provide.factory('files-service', [ "$q", function ($q) {
            ...
            ret.getRecords = function () {
                return $q.when(localforage.keys().then(function (keys) {
                        return $q.all(keys.map(function (key) {
                            return localforage.getItem(key);
                        }));
                }));
                // When the version with iterate is released the code
                // above should become:
            ...
            ret.deleteRecord = function (record) {
                ...
                //
                // We're basically polling until the record is gone.
                //
                return $q(function (resolve, reject) {
                    localforage.removeItem(record.name).then(function () {
                        function check() {
                            localforage.getItem(record.name).then(
                                function (item) {
                                if (item !== null)
