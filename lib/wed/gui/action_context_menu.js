 ...
 */
define(/** @lends module:gui/action_context_menu */ function f(require,
                                                               exports) {
  "use strict";
  ...
  function makeKindHandler(me, kind) {
    return log.wrap(function handler() {
  ...
  ContextMenu.prototype.handleToggleFocus = function handleToggleFocus() {
    this._action_filter_input.focus();
  };

  var ITEM_SELECTOR = "li:not(.divider):visible a";

  ContextMenu.prototype._actionKeydownHandler = log.wrap(
    function _actionKeydownHandler(ev) {
  ...
  ContextMenu.prototype._actionKeypressHandler = log.wrap(
    function _actionKeypressHandler(ev) {
      ...
      for (var i = 0; i < KEY_TO_FILTER.length; ++i) {
        var spec = KEY_TO_FILTER[i];
        var key = spec.key;
  ...
  ContextMenu.prototype._inputChangeHandler = log.wrap(
    function _inputChangeHandler(ev) {
  ...
  ContextMenu.prototype._inputKeydownHandler = log.wrap(
  ...
  ContextMenu.prototype._render = function _render() {
    var menu = this._menu;
  ...
  ContextMenu.prototype._computeActionItemsToDisplay =
    function _computeActionItemsToDisplay(items) {
      ...
      switch (kind_filter) {
      case null:
        kindMatch = function kindMatchAll() {
          return true;
        };
        break;
      case undefined:
      ...
      switch (type_filter) {
      case null:
        typeMatch = function typeMatchAll() {
          return true;
        };
        break;
      case undefined:
        typeMatch = function typeMatchOther(item) {
          return !item.action || TYPES.indexOf(item.action.nodeType) === -1;
        };
        break;
      default:
        typeMatch = function typeMatchSome(item) {
          return item.action && item.action.nodeType === type_filter;
        };
      }
      ...
      if (text_filter) {
        ...
        textMatch = function textMatchAll() {
          return true;
        };
      }
