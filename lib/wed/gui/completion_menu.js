 ...
 */
define(/** @lends module:gui/completion_menu */ function f(require, exports) {
  "use strict";
  ...
  CompletionMenu.prototype._globalKeydownHandler =
    function _globalKeydownHandler(wed_ev, ev) {
  ...
  CompletionMenu.prototype._render = function _render() {
    var items = [];
    ...
    var completion_items = this._completion_items;
    for (var i = 0; i < completion_items.length; ++i) {
      var item = completion_items[i];
      if (prefix === "") {
    ...
    if (items.length === 1 && completion_items[0] === prefix) {
      this.dismiss();
    }
  ...
  CompletionMenu.prototype.dismiss = function dismiss() {
    if (this._dismissed) {
      return;
    }
