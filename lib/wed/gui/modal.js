...
/**
 * <p>A modal needs to be created only once per instance of wed. After
...
<div class="modal" style="position: absolute" tabindex="1">\
  <div class="modal-dialog">\
    ...
    this._$dom.on('click', '.btn', jqutil.eventHandler(function (ev, jQthis) {
        this._$clicked = $(jQthis);
        return true;
    }.bind(this)));

    this._$dom.on('shown.bs.modal.modal',
                  jqutil.eventHandler(this._handleShown.bind(this)));
 ...
 * itself. The callback left out if the modal is merely for
 * informational purposes.
 */
Modal.prototype.modal = function (callback) {
    this._$clicked = undefined;
    var bound = jqutil.eventHandler(handler.bind(this));
    function handler (ev, jQthis) {
        callback(ev, jQthis);
        /*jshint validthis:true */
        this._$dom.off("hide.bs.modal.modal", bound);
    }
    if (callback)
        this._$dom.on("hide.bs.modal.modal", bound);
    this._$dom.modal();
};
...
Modal.prototype._handleShown = function (ev, jQthis) {
    var height = this._$dom.parent().height();
    this._$body.css("max-height", height - this._$header.height() - this._$footer.height());
};
