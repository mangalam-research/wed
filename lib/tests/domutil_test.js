...
define(function f(require) {
  ...
  var commonSource = "./domutil_test_data/source_converted.xml";
  ...
  describe("domutil", function domutilBlock() {
    ...
    describe("splitTextNode", function splitTextNode() {
      ...
      beforeEach(function beforeEach(done) {
        require(["text!" + commonSource], function loaded(data) {
...
._local_q._xmlns_http\\:\\/\\/www\\.tei-c\\.org\\/ns\\/1\\.0._real");
    ...
    describe("dataFind/dataFindAll", function dataFind() {
      var source = "./domutil_test_data/dataFind_converted.xml";
      var data_root;
      var gui_root;
      before(function before(done) {
    ...
    describe("contains", function contains() {
      var ul;
      var li;
      ...
      it("handles elements", function test() {
        assert.isTrue(domutil.contains(ul, li));
        assert.isFalse(domutil.contains(li, ul));
      });

      it("handles attributes", function test() {
        assert.isTrue(domutil.contains(li, li.attributes.class));
        // Transitively: the attribute is contained by li, which is contained by
        // ul
        assert.isTrue(domutil.contains(ul, li.attributes.class));
        assert.isFalse(domutil.contains(li.attributes.class, ul));
      });
    });
  });
});
