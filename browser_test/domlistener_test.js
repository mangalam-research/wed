...
//
// Could be moved to jsdom-based tests. Old code depended on MutationObserver
// which does not exist in jsdom. But the newer implementation does not.
//

define(function f(require) {
  "use strict";
  var dloc = require("wed/dloc");
  var domlistener = require("wed/domlistener");
  var TreeUpdater = require("wed/tree-updater").TreeUpdater;
  ...
  function Mark(total_expected, counts, listener, done) {
    this._count = 0;
  ...
  describe("domlistener", function mainBlock() {
    ...
    beforeEach(function beforeEach() {
      ...
      $domroot.empty();
      $domroot.append("<div></div>");
      root = $domroot[0].firstElementChild;
      new dloc.DLocRoot(root); // eslint-disable-line no-new
      $root = $(root);
      tree_updater = new TreeUpdater(root);
      listener = new Listener(root, tree_updater);
    });
    ...
    it("fires included-element, added-element and children-changed when " +
       ...
       function test(done) {
         ...
         tree_updater.insertNodeAt(root, root.childNodes.length,
                                   $fragment_to_add[0]);
         mark.check();
       });
    ...
    it("fires text-changed when changing a text node", function test(done) {
      ...
      $root.append($fragment_to_add);
      listener.startListening();
      tree_updater.setTextNodeValue($root.find("._real.li")[0].firstChild, "Q");
      mark.check();
    });
    ...
    it("generates children-changing and children-changed with " +
       ...
       function test(done) {
         ...
         listener.addHandler("children-changing", "._real.ul", changingHandler);
         function changedHandler(this_root, added, removed,
                                 previous_sibling, next_sibling,
                                 element) {
           // The marker will also trigger this handler. Ignore it.
    ...
    it("generates included-element with the right tree, and " +
       ...
       function test(done) {
         ...
         listener.startListening();
         var $ul;
         tree_updater.insertNodeAt(root, root.childNodes.length, $fragment[0]);
         $ul = $root.find(".ul");
    ...
    it("clearPending clears pending operations", function test() {
      var marked = false;
      mark = new Mark(1, { "children root": 1 }, listener, function markIt() {
        marked = true;
      });
      function changedHandler(this_root, added) {
        if (added[0] === $marker[0]) {
